...
if (!function_exists('get_file_icon')) {
    ...
    function get_file_icon($extension = "", $return_icon_name = true) {

        $extension = strtolower($extension);
        $ext_label = strtoupper(substr($extension, 0, 4));

        $groups = [

            "file-doc" => ["pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "txt", "rtf", "csv", "odt", "pps", "pot"],
            "file-image" => ["jpg", "jpeg", "png", "gif", "svg", "webp", "bmp", "tiff", "ico"],
            "file-audio" => ["mp3", "wav", "aac", "flac", "ogg", "m4a", "wma"],
            "file-video" => ["mp4", "mkv", "avi", "mov", "wmv", "flv", "webm", "mpg", "mpeg", "3gp"],
            "file-archive" => ["zip", "rar", "7z", "tar", "gz", "bz2", "xz", "s7z", "iso"],
            "file-code" => ["html", "css", "js", "php", "py", "java", "c", "cpp", "cs", "go", "rb", "sh"],
            "file-data" => ["sql", "db", "sqlite", "json", "xml", "yaml", "yml"],
            "file-app" => ["exe", "msi", "apk", "ipa", "app", "deb", "rpm"],
            "file-design" => ["psd", "ai", "xd", "sketch", "fig", "indd", "aep", "prproj", "blend", "cdr"],
            "file-cad" => ["dwg", "dxf", "ifc", "rvt", "rfa", "skp", "3ds", "obj", "stl", "step", "iges"]
        ];
        ...
        foreach ($groups as $group_class => $extensions) {
        ...
        $icon_html = "<div class='icon-wrapper'>
                        <span class='file-icon " . $class . "' data-ext='" . $ext_label . "'>
                            <i data-feather='file' class='mr10 float-start icon-40 bold-file-icon'></i>
                        </span>
                    </div>";
 ...
 * 
 * @param mixed $file File object, array, or string
 * @param string $file_path
 * @param bool $convert_to_base64 Return base64 encoded image instead of URL
 * @param bool $only_file_path_with_slash Return only file path with slash
 * @return string Thumbnail URL or base64 encoded image
 */
if (!function_exists('get_file_thumbnail_preview')) {

    function get_file_thumbnail_preview($file, $file_path = "", $convert_to_base64 = false, $only_file_path_with_slash = false) {
        if (!$file) {
            return "";
        }
        ...
        if (is_object($file)) {
            $file_data = [
                ...
                "service_type" => $file->service_type ?? null
            ];
        ...
        $cache_file_identifier = !empty($file_data['file_id']) ? $file_data['file_id'] : $file_data['file_name'];
        $cache_name = "file_thumb_v2_" . md5(
            ($file_data['service_type'] ?? 'local') . '_' .
                $cache_file_identifier . '_' .
                $file_data['file_name'] . '_' .
                $file_path
        );

        if ($convert_to_base64 === false) {
            return get_source_url_of_file($file_data, $file_path, "thumbnail", $only_file_path_with_slash, $only_file_path_with_slash, true);
        }

        if (!$cache_file = cache($cache_name)) {

            $thumbnail_url = get_source_url_of_file($file_data, $file_path, "thumbnail", $only_file_path_with_slash, $only_file_path_with_slash, true);
            ...
            $ext = strtolower(pathinfo($file_data['file_name'], PATHINFO_EXTENSION));

            $cache_file = 'data:image/' . $ext . ';base64,' . base64_encode($data);
            cache()->save($cache_name, $cache_file, 3600);
        }
...
if (!function_exists('get_ai_files_path')) {

    function get_ai_files_path($ai_service) {
        if ($ai_service) {
            $target_path = get_setting("ai_files_path");
            if (!$target_path) {
                $target_path = 'files/ai/';
            }

            return $target_path . $ai_service . "/";
        }
    }
}
