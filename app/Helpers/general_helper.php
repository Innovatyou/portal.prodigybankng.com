...
if (!function_exists('get_invoice_making_data')) {
    ...
    function get_invoice_making_data($invoice_id) {
        ...
        if ($invoice_info) {
            $period = get_invoice_period($invoice_info);

            $items = $ci->Invoice_items_model->get_details(array("invoice_id" => $invoice_id))->getResult();

            // Replace {PERIOD} in item titles and descriptions
            foreach ($items as &$item) {
                if ($period) {
                    if (strpos($item->title, '{PERIOD}') !== false) {
                        $item->title = str_replace('{PERIOD}', $period, $item->title);
                    }

                    if (strpos($item->description, '{PERIOD}') !== false) {
                        $item->description = str_replace('{PERIOD}', $period, $item->description);
                    }
                }
            }
...
if (!function_exists('prepare_estimate_pdf')) {
    ...
    function prepare_estimate_pdf($estimate_data, $mode = "download", $is_mobiel_preview = false) {
        ...
        if ($estimate_data) {
            ...
            return $pdf->PreparePDF($html, $estimate_info->display_id, $mode, $is_mobiel_preview);
        }
    }
}
...
if (!function_exists('prepare_order_pdf')) {
    ...
    function prepare_order_pdf($order_data, $mode = "download", $is_mobiel_preview = false) {
        ...
        if ($order_data) {
            ...
            $pdf = new Pdf("order");
            return $pdf->PreparePDF($html, $order_info->display_id, $mode, $is_mobiel_preview);
        }
    }
}
...
if (!function_exists('prepare_contract_view')) {
    ...
    function prepare_contract_view($contract_data) {
        if ($contract_data) {
            ...
            $parser_data["CONTRACT_ID"] = $contract_info->display_id;
...
if (!function_exists('prepare_proposal_view')) {
    ...
    function prepare_proposal_view($proposal_data) {
        if ($proposal_data) {
            ...
            $parser_data["PROPOSAL_ID"] = $proposal_info->display_id;
            ...
            if ($proposal_info->status === "accepted" && ($signer_info || $proposal_info->accepted_by)) {
                $parser_data["CLIENT_SIGNER_NAME"] = $proposal_info->accepted_by ? $proposal_info->signer_name : get_array_value($signer_info, "name");
                $parser_data["CLIENT_SIGNER_EMAIL"] = $proposal_info->signer_email ? $proposal_info->signer_email : get_array_value($signer_info, "email");
...
if (!function_exists('create_invoice_from_subscription')) {
    ...
    function create_invoice_from_subscription($subscription_id) {
        ...
        if ($subscription_info) {
            ...
            $current_next_recurring_date = $subscription_info->next_recurring_date;
            $base_date = $current_next_recurring_date ? $current_next_recurring_date : $bill_date;
            $next_recurring_date = add_period_to_date($base_date, $subscription_info->repeat_every, $subscription_info->repeat_type);
            ...
            $period_end = subtract_period_from_date($next_recurring_date, 1, "days");

            $period_meta = serialize(array(
...
if (!function_exists('prepare_proposal_pdf')) {
    ...
    function prepare_proposal_pdf($proposal_data, $mode = "download", $is_mobiel_preview = false) {
        if ($proposal_data) {
            ...
            return $pdf->PreparePDF($html, $proposal_info->display_id, $mode, $is_mobiel_preview);
        }
    }
}
...
if (!function_exists('prepare_contract_pdf')) {
    ...
    function prepare_contract_pdf($contract_data, $mode = "download", $is_mobiel_preview = false) {
        ...
        if ($contract_data) {
            ...
            return $pdf->PreparePDF($html, $contract_info->display_id, $mode, $is_mobiel_preview);
        }
    }
}
...
if (!function_exists('send_stream_message_to_the_browser')) {

    function send_stream_message_to_the_browser($content) {
        echo $content;
...
/**
 * get next display id for a given type
 * 
 * @param string $type
 * @return array
 */
if (!function_exists('get_next_display_id')) {
    function get_next_display_id($type) {
        $ci = new App_Controller();

        $config = [
            "order" => [
                "model" => "Orders_model",
                "method" => "get_order_last_id",
                "prefix_setting" => "order_prefix",
                "initial_setting" => "initial_number_of_the_order"
            ],
            "estimate" => [
                "model" => "Estimates_model",
                "method" => "get_estimate_last_id",
                "prefix_setting" => "estimate_prefix",
                "initial_setting" => "initial_number_of_the_estimate"
            ],
            "proposal" => [
                "model" => "Proposals_model",
                "method" => "get_proposal_last_id",
                "prefix_setting" => "proposal_prefix",
                "initial_setting" => "initial_number_of_the_proposal"
            ],
            "contract" => [
                "model" => "Contracts_model",
                "method" => "get_contract_last_id",
                "prefix_setting" => "contract_prefix",
                "initial_setting" => "initial_number_of_the_contract"
            ],
            "subscription" => [
                "model" => "Subscriptions_model",
                "method" => "get_last_subscription_id",
                "prefix_setting" => "subscription_prefix",
                "initial_setting" => "initial_number_of_the_subscription"
            ],
            "ticket" => [
                "model" => "Tickets_model",
                "method" => "get_last_ticket_id",
                "prefix_setting" => "ticket_prefix",
        ...
        $prefix_setting = $config[$type]["prefix_setting"];
        $initial_setting = $config[$type]["initial_setting"];

        $last_sequence = $ci->$model->$method();
        $next_number = $last_sequence + 1;

        if ($initial_setting) {
            $initial = get_setting($initial_setting);
            if ($initial > $next_number) {
                $next_number = $initial;
            }
        }

        $prefix = get_setting($prefix_setting);

        if (!$prefix) {
            $prefix = strtoupper(app_lang($type)) . " #";
        }

        return ["display_id" => $prefix . $next_number];
    }
}
 ...
 * @return string
 */
if (!function_exists('get_invoice_period')) {
    function get_invoice_period($invoice) {
        if (empty($invoice->meta_data)) {
            return "";
        }

        $meta = unserialize($invoice->meta_data);

        if (is_array($meta) && get_array_value($meta, 'period_start') && get_array_value($meta, 'period_end')) {
            return format_to_date(get_array_value($meta, 'period_start'), false) . " " . app_lang("to") . " " . format_to_date(get_array_value($meta, 'period_end'), false);
        }
...
if (!function_exists('get_task_status_name')) {
    function get_task_status_name($status_id) {
        ...
        $ci = new App_Controller();
        $status = $ci->Task_status_model->get_one($status_id);

        if ($status->key_name) {
            return app_lang($status->key_name);
        } else {
            return $status->title;
        }
    }
