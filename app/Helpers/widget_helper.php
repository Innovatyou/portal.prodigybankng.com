...
if (!function_exists('activity_logs_widget')) {
    ...
    function activity_logs_widget($params = array()) {
        ...
        $params["can_comment_on_projects"] = get_array_value($ci->login_user->permissions, "can_manage_all_projects")  || get_array_value($ci->login_user->permissions, "can_comment_on_projects");
...
if (!function_exists('projects_overview_widget')) {
    ...
    function projects_overview_widget($return_data = false) {
        $ci = new Security_Controller(false);
        ...
        $total_points = $view_data["projects_info"]->total_points;
        $completed_points = $view_data["projects_info"]->completed_points;

        $progress = $total_points ? round(($completed_points / $total_points) * 100) : 0;
        $view_data["progress"] = $progress;
...
if (!function_exists('tasks_overview_widget')) {
    ...
    function tasks_overview_widget($type = "", $return_data = false) {
        $ci = new Security_Controller(false);
        $permissions = $ci->login_user->permissions;
        ...
        $task_statistics =  $ci->Tasks_model->get_task_statistics($options);
        $view_data["task_statuses"] = $task_statistics->task_statuses;
        $view_data["task_priorities"] = $task_statistics->task_priorities;
        $view_data["expired_tasks"] = $task_statistics->expired_tasks;
        $view_data["type"] = $type;
...
if (!function_exists('invoice_overview_widget')) {
    ...
    function invoice_overview_widget($options = array()) {
        ...
        $view_data["exclude_draft"] = get_array_value($options, "exclude_draft");
        $view_data["dashboard_widget"] = get_array_value($options, "dashboard_widget");
...
/**
 * get wallet balance widget
 * @return html
 */
if (!function_exists('wallet_balance_widget')) {

    function wallet_balance_widget() {
        $ci = new Security_Controller(false);

        $Client_wallet_model = model("App\Models\Client_wallet_model");
        $client_wallet_summary = $Client_wallet_model->get_client_wallet_summary($ci->login_user->client_id);
        ...
        $template = new Template();
        return $template->view("clients/widgets/wallet_balance_widget", $view_data);
    }
}

/**
 * get request an estimate widget
 * @return html
 */
if (!function_exists('request_an_estimate_widget')) {

    function request_an_estimate_widget() {
        $template = new Template();
        return $template->view("estimate_requests/request_an_estimate_widget");
    }
}

/**
 * get total estimates widget
 * @return html
 */
if (!function_exists('total_estimates_widget')) {

    function total_estimates_widget() {
        $ci = new Security_Controller(false);
        ...
        $view_data["total_estimates"] = $ci->Estimates_model->count_estimates($options);

        $template = new Template();
        return $template->view("estimates/total_estimates_widget", $view_data);
    }
}

/**
 * get contracts expiring soon widget
 * 
 * @return html
 */
if (!function_exists('contracts_expiring_soon_widget')) {

    function contracts_expiring_soon_widget() {
        $template = new Template();
        return $template->view("contracts/contracts_expiring_soon_widget");
    }
}
