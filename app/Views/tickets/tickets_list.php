...
foreach ($ticket_statuses as $status) {
    ...
    if (isset($selected_client_id) && $selected_client_id) {
        // When a client is selected, check all statuses
        $is_selected = true;
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var batchDeleteUrl = "<?php echo_uri('tickets/delete_selected_tickets'); ?>";
        ...
        $("#ticket-table").appTable({
            source: '<?php echo_uri("tickets/list_data/0/") ?>' + mobileView + "/" + clientId,
            serverSide: true,
            ...
            printColumns: combineCustomFieldsColumns([2, 3, 4, 5, 6, 7, 8, 11, 12, 13], '<?php echo $custom_field_headers; ?>'),
            xlsColumns: combineCustomFieldsColumns([2, 3, 4, 5, 6, 7, 8, 11, 12, 13], '<?php echo $custom_field_headers; ?>')
        });
    });
</script>