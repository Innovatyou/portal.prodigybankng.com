...
$is_edit = $model_info->id;

if ($is_edit) {
    $log_type = $model_info->context;
} else {
    $log_type = $selected_type;
}

$is_project_only = $show_project_timesheet && !$show_general_task_timesheet && !$show_ticket_timesheet;
?>

<?php echo form_open(get_uri("timesheets/save"), array("id" => "timelog-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <div class="container-fluid">
        <input type="hidden" name="id" value="<?php echo $model_info->id; ?>" />
        <input type="hidden" name="project_id" value="<?php echo $project_id; ?>" />
        <input type="hidden" name="context" value="<?php echo $log_type; ?>" />

        <?php if (!$model_info->id && !$project_id && !$is_project_only) { ?>
            <div class="form-group">
                <div class="row">
                    <label for="context" class="col-md-3"><?php echo app_lang('type'); ?></label>
                    <div class="col-md-9">
                        <?php if ($show_project_timesheet) { ?>
                            <?php
                            ...
                            ), "project", $log_type === "project");
                            ?>
                            <label for="project" class="mr15"><?php echo app_lang('project_timesheet'); ?></label>
                        <?php } ?>
                        <?php if ($show_general_task_timesheet) { ?>
                            <?php
                            echo form_radio(array(
                                "id" => "general_task",
                                "name" => "context",
                                "class" => "form-check-input",
                                "data-msg-required" => app_lang("field_required"),
                            ), "general_task", $log_type === "general_task");
                            ?>
                            <label for="general_task" class="mr15"><?php echo app_lang('general_task_timesheet'); ?></label>
                        <?php } ?>
                        <?php if ($show_ticket_timesheet) { ?>
                            <?php
                            echo form_radio(array(
                                "id" => "ticket",
                                "name" => "context",
                                "class" => "form-check-input",
                                "data-msg-required" => app_lang("field_required"),
                            ), "ticket", $log_type === "ticket");
                            ?>
                            <label for="ticket"><?php echo app_lang('ticket_timesheet'); ?></label>
                        <?php } ?>
                    </div>
                </div>
            </div>
        ...
        <?php if (!$project_id) { ?>
            <div class="form-group log-type-project" id="project_id_wrapper">
                <div class="row">
        ...
        <div class="form-group log-type-ticket" id="ticket_id_wrapper">
            <div class="row">
                <label for="ticket_id" class="col-md-3"><?php echo app_lang('ticket'); ?></label>
                <div class="col-md-9">
                    <?php
                    echo form_input(array(
                        "id" => "ticket_id",
                        "name" => "ticket_id",
                        "value" => $model_info->context_id,
                        "class" => "form-control",
        ...
        <div class="form-group log-type-project log-type-general_task" id="task_id_wrapper">
            <div class="row">
...
<script type="text/javascript">
    $(document).ready(function() {
        var TIMESHEET = {
            ...
            type: "<?php echo $log_type; ?>",
            projectId: "<?php echo $project_id; ?>",
            taskId: "<?php echo $model_info->task_id; ?>",
            ticketId: "<?php echo $model_info->context_id; ?>",
            userId: "<?php echo $model_info->user_id; ?>"
        };

        $("#timelog-form").appForm({
            onSuccess: function(result) {
                var table = $(".dataTable:visible").attr("id");
                if (table === "project-timesheet-table" || table === "all-timesheet-table") {
                    $("#" + table).appTable({
                        newData: result.data,
                        dataId: result.id
                    });
        ...
        $("#project_id").select2().on("change", function() {
            ...
            if (projectId) {
                ...
                appAjaxRequest({
                    url: "<?php echo get_uri('timesheets/get_all_related_data_of_selected_project_for_timelog') ?>" + "/" + projectId,
                    dataType: "json",
                    success: function(result) {
                        $("#user_id").show().val("");
                        $('#user_id').select2({
                            data: result.project_members_dropdown
                        });
                        $("#task_id").show().val("");
                        $('#task_id').select2({
                            data: result.tasks_dropdown
                        });
                        appLoader.hide();
                    }
                });
        ...
        $("#user_id").select2({
            data: <?php echo json_encode($project_members_dropdown); ?>
        });
        $("#task_id").select2({
            data: <?php echo $tasks_dropdown; ?>
        });

        $("#ticket_id").select2({
            data: <?php echo $tickets_dropdown; ?>
        });
        ...
        function applyTimelogType(type) {
            $("#project_id_wrapper, #task_id_wrapper, #ticket_id_wrapper").hide();

            $("#project_id, #task_id, #ticket_id").removeClass("validate-hidden").removeAttr("data-rule-required data-msg-required");

            if (TIMESHEET.isEdit) {
                if (type === "project") {
                    $("#task_id_wrapper").show();
                }
            ...
            if (type === "project") {
                $("#project_id_wrapper, #task_id_wrapper").show();

                $("#project_id").addClass("validate-hidden").attr({
                    "data-rule-required": true,
                    "data-msg-required": "<?php echo app_lang('field_required'); ?>"
                });

                loadProjectData(TIMESHEET.projectId);
            }
            ...
            if (type === "general_task") {
                ...
                $("#task_id").addClass("validate-hidden").attr({
                    "data-rule-required": true,
                    "data-msg-required": "<?php echo app_lang('field_required'); ?>"
                });
            }
            ...
            if (type === "ticket") {
                ...
                $("#ticket_id").addClass("validate-hidden").attr({
                    "data-rule-required": true,
                    "data-msg-required": "<?php echo app_lang('field_required'); ?>"
                });
            }
        ...
        function loadProjectData(projectId) {
            $("#user_id, #task_id").select2("destroy");
            ...
            $("#user_id, #task_id").select2("destroy").hide();
            appLoader.show({
                container: "#dropdown-apploader-section"
            });
            ...
            appAjaxRequest({
                ...
                success: function(result) {
                    ...
                    $("#user_id").show().val("").select2({
                        data: result.project_members_dropdown
                    });

                    $("#task_id").show().val("").select2({
                        data: result.tasks_dropdown
                    });
        ...
        function loadGeneralTaskData() {
            appLoader.show({
                container: "#dropdown-apploader-section"
            });

            appAjaxRequest({
                url: "<?php echo get_uri('timesheets/get_general_task_related_data'); ?>",
                dataType: "json",
                success: function(result) {

                    $("#user_id").show().val("").select2({
                        data: result.members_dropdown
                    });
                    ...
                    if (TIMESHEET.isEdit) {
                        if (TIMESHEET.taskId) {
                            $("#task_id").val(TIMESHEET.taskId).trigger("change");
                        }
                    }
        ...
        applyTimelogType(TIMESHEET.type);

        if (!TIMESHEET.isEdit) {
            $("input[name='context']").on("change", function() {
                applyTimelogType(this.value);
            });
        }
    });
</script>