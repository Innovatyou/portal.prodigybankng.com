...
?>
    ...
    <div class="card border-top-0 rounded-top-0">
        <ul id="project-all-timesheet-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title scrollable-tabs" role="tablist">
            ...
            <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("timesheets/all_timesheet_summary/$user_id"); ?>" data-bs-target="#timesheet-summary"><?php echo app_lang('summary'); ?></a></li>
            <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("timesheets/chart/0/$user_id"); ?>" data-bs-target="#timesheet-chart"><?php echo app_lang('chart'); ?></a></li>
            <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("timesheets/daily_timesheet_activity/0/$user_id"); ?>" data-bs-target="#daily-activity"><?php echo app_lang('daily_activity'); ?></a></li>
        </ul>

        <div class="tab-content">
            <div role="tabpanel" class="tab-pane fade" id="timesheet-details">
                <div class="table-responsive">
                    <table id="all-timesheet-table" class="display" width="100%">
                    </table>
                </div>
            </div>
...
<?php } ?>
        ...
        var isEnabledTicketTimesheet = <?php echo get_setting("module_ticket_timesheet") ? "true" : "false"; ?>;
        var isEnabledProjectTimesheet = <?php echo get_setting("module_project_timesheet") ? "true" : "false"; ?>;
        var isEnabledGeneralTaskTimesheet = <?php echo get_setting("module_general_task_timesheet") ? "true" : "false"; ?>;

        var taskTitleColumn = "<?php echo app_lang('task') ?>";

        var hasTaskModule = isEnabledProjectTimesheet || isEnabledGeneralTaskTimesheet;

        if (isEnabledTicketTimesheet && !hasTaskModule) {
            taskTitleColumn = "<?php echo app_lang('ticket') ?>";
        } else if (isEnabledTicketTimesheet && hasTaskModule) {
            taskTitleColumn = "<?php echo app_lang('task') ?> / <?php echo app_lang('ticket') ?>";
        } else if (hasTaskModule) {
            taskTitleColumn = "<?php echo app_lang('task') ?>";
        }

        $("#all-timesheet-table").appTable({
            source: '<?php echo_uri("timesheets/list_data/" . $user_id) ?>',
            serverSide: true,
            ...
            columns: [
                ...
                {title: taskTitleColumn, order_by: "task_title"},
