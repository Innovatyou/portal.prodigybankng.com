...
if ($notification->event === "project_task_updated" && $notification->activity_log_type === "task" && !empty($notification->activity_log_changes)) {

    $changes = unserialize($notification->activity_log_changes);

    if (!empty($changes["status_id"])) {

        $from_id = $changes["status_id"]["from"] ?? null;
        $to_id   = $changes["status_id"]["to"] ?? null;

        if ($from_id && $to_id) {
            $from = get_task_status_name($from_id);
            $to   = get_task_status_name($to_id);

            echo "\n* " . app_lang("status") . ":* {$from} → {$to}";
        }
    }
}

if ($notification->payment_invoice_id) {
    echo "\n" . to_currency($notification->payment_amount, $notification->client_currency_symbol) . "  -  " . $notification->payment_invoice_display_id;
}

if ($notification->ticket_id && $notification->ticket_title) {
    echo "\n" . $notification->ticket_display_id . " - " . $notification->ticket_title;
}
...
if ($notification->estimate_id) {
    echo "\n" . $notification->estimate_display_id;
}
