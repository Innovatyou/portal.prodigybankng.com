<?php echo form_open(get_uri($controller_slag . "/save_from_excel_file"), array("id" => "import-form", "class" => "general-form", "role" => "form")); ?>
    ...
    <div class="container-fluid">
        <div id="upload-area">
            ...
            echo view("includes/multi_file_uploader", array(
                "upload_url" => get_uri($controller_slag . "/upload_excel_file"),
                "validation_url" => get_uri($controller_slag . "/validate_import_file"),
                "max_files" => 1,
                "hide_description" => (isset($show_description) && $show_description) ? false : true,
...
<div class="modal-footer">
    <?php echo anchor($controller_slag . "/download_sample_excel_file", "<i data-feather='download' class='icon-16'></i> " . app_lang("download_sample_file"), array("title" => app_lang("download_sample_file"), "class" => "btn btn-default float-start")); ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#import-form").appForm({
            onSuccess: function() {
                if (!$("#ajaxModalContent").find(".modal-body").attr("data-trigger-on-modal-close")) {
                    location.reload();
                }
            }
        });
        ...
        $nextButton.click(function() {
            ...
            appAjaxRequest({
                url: "<?php echo get_uri($controller_slag . '/validate_import_file_data') ?>",
                type: 'POST',
                dataType: 'json',
                data: {
                    file_name: fileName
