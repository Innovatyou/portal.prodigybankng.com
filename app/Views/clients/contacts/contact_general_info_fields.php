...
<div class="container-fluid">
    ...
    <input type="hidden" name="is_existing_contact" value="" />
    ...
    <?php if (!empty($is_modal)) { ?>
        ...
        <div class="form-group">
            <div class="row">
                <label for="contact_type" class="<?php echo $label_column; ?>"><?php echo app_lang('contact_type'); ?></label>
                <div class="<?php echo $field_column; ?>">
                    ...
                    ?>
                    <label for="contact_type_new" class="mr15 p0"><?php echo app_lang('new'); ?></label>
                    <?php
                    echo form_radio(array(
                        "id" => "contact_type_existing",
                        "name" => "contact_type",
                        "class" => "form-check-input",
                    ), "existing", false);
                    ?>
                    <label for="contact_type_existing" class="p0 mr15"><?php echo app_lang('select_existing'); ?></label>
                </div>
            </div>
        </div>

        <div class="form-group" id="existing_contact_wrapper" style="display:none;">
            <div class="row">
                <label for="select_existing" class="<?php echo $label_column; ?>"><?php echo app_lang('select_existing'); ?></label>
                <div class="<?php echo $field_column; ?>">
                    <?php
                    echo form_input(array(
                        ...
                        "value" => "",
                        "class" => "form-control",
                        "placeholder" => app_lang('select_existing'),
    ...
    $(document).ready(function() {
        ...
        <?php if (!empty($is_modal)) { ?>
            ...
            $('#select_existing').select2({
                data: <?php echo json_encode($existing_contacts_dropdown); ?>
            });
            ...
            $("input[name='contact_type']").change(function() {
                if ($(this).val() === "existing") {
                    ...
                    $("input[name='is_existing_contact']").val(1);
                } else {
                    $("#existing_contact_wrapper").hide();
                    $("#new_contact_fields").show();
                    $("input[name='is_existing_contact']").val(0);
                }
            });
            ...
            $("#select_existing").change(function() {
                ...
                $.ajax({
                    url: "<?php echo get_uri('clients/get_contact_details'); ?>",
                    data: {
                        id: contact_id
                    },
                    type: "POST",
                    ...
                    success: function(response) {
                        if (response && response.success) {
                            var contactInfo = response.contact_info;
                            ...
                            if (!contactInfo.gender) {
                                ...
                                $("input[name='gender'][value='" + contactInfo.gender + "']").prop("checked", true);
                            }
                        }
                    }
                });
