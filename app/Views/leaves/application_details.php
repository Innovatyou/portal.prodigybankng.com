    ...
    <div class="row">
        ...
        <div class="table-responsive mb15">
            <table class="table dataTable display b-t">
                <tr>
                    <td class="w150"> <?php echo app_lang('leave_type'); ?></td>
                ...
                <?php if ($leave_info->status === "rejected") { ?>
                    ...
                    <tr>
                        <td> <?php echo app_lang('rejection_reason'); ?></td>
                        <td><?php echo nl2br($leave_info->rejection_reason ? $leave_info->rejection_reason : "-"); ?></td>
                    </tr>
                <?php } ?>
...
<div class="modal-footer">
    ...
    <?php if ($leave_info->status === "pending" && $login_user->id === $leave_info->applicant_id) { ?>
        <button data-status="canceled" type="submit" class="btn btn-danger update-leave-status"><span data-feather="x-circle" class="icon-16"></span> <?php echo app_lang('cancel'); ?></button>
    <?php } ?>
    <?php if ($leave_info->status === "pending" && $show_approve_reject) {
        echo modal_anchor(get_uri("leaves/reject_reason_modal_form/" . $leave_info->id), "<i data-feather='x-circle' class='icon-16'></i> " . app_lang('reject'), array("class" => "btn btn-danger", "title" => app_lang('reject_reason')));
    ?>
        <button data-status="approved" type="submit" class="btn btn-success update-leave-status"><span data-feather="check-circle" class="icon-16"></span> <?php echo app_lang('approve'); ?></button>
    <?php } ?>
</div>
<?php echo form_close(); ?>
