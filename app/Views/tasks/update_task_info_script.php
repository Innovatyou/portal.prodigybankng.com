            ...
            $(this).appModifier({
                ...
                onSuccess: function(response, newValue) {
                    if (response.success) {
                        ...
                        // Update kanban card only
                        pendingKanbanRefresh = response.id;
                        $('#ajaxModal').off('hidden.bs.modal').one('hidden.bs.modal', function(e) {
                            if (typeof updateKanbanCard === "function" && pendingKanbanRefresh) {
                                refreshKanbanCard(pendingKanbanRefresh);
                                pendingKanbanRefresh = null;
                            }
                        });
