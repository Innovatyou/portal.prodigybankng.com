        ...
        <div class="col-sm-9 col-lg-10">
            ...
            <div class="card clearfix">
                ...
                <ul id="custom-field-tab" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title scrollable-tabs" role="tablist">
                    <li class="title-tab">
                        <h4 class="pl15 pt10 pr15"><?php echo app_lang("custom_fields"); ?></h4>
                    </li>
                    ...
                    if ($project_timesheet || $ticket_timesheet || $general_task_timesheet) { ?>
                    ...
                    <div role="tabpanel" class="tab-pane fade clearfix" id="custom-field-clients">
                        <div class="mb0 p20">
                            <div class="table-responsive general-form">
                                <table id="custom-field-table-clients" class="" cellspacing="0" width="100%">
                                </table>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('body').on('click', 'a[data-action=delete-confirmation]', function(e) {
    ...
    loadCustomFieldTable = function(relatedTo) {
        ...
        appAjaxRequest({
            url: '<?php echo_uri("custom_fields/list_data") ?>' + "/" + relatedTo,
            type: "POST",
            dataType: "json",
            success: function(response) {

                $("#custom-field-table-" + relatedTo).addClass("display no-thead b-t b-b-only no-hover dataTable no-footer").append("<tbody id='custom-field-table-sortable-" + relatedTo + "'></tbody>");

                $.each(response.data, function(key, value) {
                    var row = `<tr><td class='w25'>${value[0]}</td><td>${value[1]}</td><td class='text-right option w100'>${value[3]}</td></tr>`;
                    $("#custom-field-table-sortable-" + relatedTo).append(row);
                });
