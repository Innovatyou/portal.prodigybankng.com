...
class Imap {
    ...
    private function _prepare_data($message_info) {
        ...
        // for general imap, the first email would normally have no reference id or in reply to
        // so, check if there is any message id
        // for replying email, the message id is different but there should be some references or in reply to 
        // so we need to check that inside the database
        $reference_id = $message_info->getReferences();
        if (!$reference_id) {
            $reference_id = $message_info->getInReplyTo();
        }
        if (!$reference_id) {
            $reference_id = $message_info->getId();
        }
        if ($reference_id && is_array($reference_id)) {
