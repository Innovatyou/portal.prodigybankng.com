...
class Permission_manager {
    ...
    function can_manage_tickets($ticket_id = 0) {
        ...
        if ($this->is_team_member()) {
            ...
            if ($ticket_id && ($ticket_permission == "assigned_only" || $ticket_permission == "specific")) {
                ...
                $ticket_specific_permission = get_array_value($this->permissions, "ticket_specific");
    ...
    function has_all_projects_restricted_role() {
        if ($this->is_team_member() && !$this->is_admin() && get_array_value($this->permissions, "do_not_show_projects") == "1") {
            return true;
        }
    }

    function can_manage_all_projects() {
        if ($this->is_admin() || get_array_value($this->permissions, "can_manage_all_projects") == "1") {
            return true;
        }
    }

    function can_create_notes() {
        if (!$this->is_active_module("module_note")) {
            return false;
        }

        if ($this->is_client() && !get_setting("client_can_access_notes")) {
            return false;
        }
    ...
    function can_create_todo() {
        if (!$this->is_active_module("module_todo")) {
            return false;
        }
    ...
    function can_access_quick_assistant() {
        if (get_setting("quick_assistant_ai_agent") && ($this->is_admin() || get_array_value($this->permissions, "can_access_quick_assistant"))) {
            return true;
        }
    ...
    function get_accessible_ai_agents() {
        if ($this->is_admin()) {
            return "";
        }

        $accessible_ai_agents_permission = get_array_value($this->permissions, "accessible_ai_agents_permission");
        if (!$accessible_ai_agents_permission || $accessible_ai_agents_permission === "all") {
            // can access all agents
            return "";
        }

        if ($accessible_ai_agents_permission === "specific") {
            return get_array_value($this->permissions, "accessible_ai_agents_permission_specific");
        }

        return null;
    }

    function can_access_this_ai_agent($agent_id) {

        // check agent status
        $agent = $this->ci->AI_agents_model->get_one($agent_id);

        if (!$agent || $agent->status !== "active" || $agent->deleted == 1) {
            return false;
        }
        ...
        if ($accessible_ai_agents_permission === "specific") {
            $accessible_ai_agents = get_array_value($this->permissions, "accessible_ai_agents_permission_specific");
            $accessible_ai_agents = explode(",", $accessible_ai_agents);
            if (in_array($agent_id, $accessible_ai_agents)) {
                return true;
            }
        }
    ...
    function can_access_ai_chatbox() {
        if (get_setting("ai_chatbox_agents") && ($this->is_admin() || get_array_value($this->permissions, "can_access_ai_chatbox"))) {
            return true;
        }
