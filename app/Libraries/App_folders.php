...
trait App_folders {
    ...
    private function _get_folder_window_data($folder_id = "", $id = 0, $context = "", $context_id = 0, $view_from = "") {
        ...
        foreach ($data["folder_items"] as $folder_item) {
            if (isset($folder_item->file_name)) {
                $folder_item->thumbnail_file_path = $this->_get_file_path($folder_item);
            }
        }
    ...
    private function get_folder_details($folder_id) {
        ...
        $options = $this->_preapare_folder_params($model_info->context, $model_info->context_id);
