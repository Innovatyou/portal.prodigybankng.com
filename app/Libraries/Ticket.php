...
class Ticket {
    ...
    function create_ticket_from_imap($data = array()) {
        ...
        $reference_id = get_array_value($data, "reference_id");
        ...
        $ticket_id = $this->_get_parent_ticket_id($email_subject, $reference_id);
        ...
        if ($ticket_id) {
            ...
            $ticket_data = array(
                ...
                "message_id" => $reference_id ? $reference_id : "", // save thread id as message id since no existing ticket is found for the thread id
            );
    ...
    private function _cut_html_by_separator($html) {
        // Remove tagged version
        $html = preg_replace(
            '/<[^>]*>\s*##-\s*please\s*type\s*your\s*reply\s*above\s*this\s*line\s*-##\s*<\/[^>]*>.*$/is',
            '',
            $html
        );

        // Fallback if plain text version exists
        $html = preg_replace(
            '/##-\s*please\s*type\s*your\s*reply\s*above\s*this\s*line\s*-##.*$/is',
            '',
            $html
        );
    ...
    private function _save_tickets_comment($ticket_id, $data, $client_info, $is_reply = false) {
        ...
        $description = $this->_cut_html_by_separator($description); // remove the content after ##- Please type your reply above this line -##
    ...
    private function _get_parent_ticket_id($subject = "", $reference_id = "") {

        // reference id has the first priority, check if it exists or matched
        if ($reference_id) {
            $existing_ticket_info = $this->Tickets_model->get_one_where(array("message_id" => $reference_id, "deleted" => 0));
            if ($existing_ticket_info->id) {
                return $existing_ticket_info->id;
            }
        }

        // if reference id is not found, check the subject
        if (!$subject) {
            return false;
        }
