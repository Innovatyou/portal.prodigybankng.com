...
trait Excel_import {
    ...
    function import_modal_form() {
        ...
        $view_data["agent_id"] = $this->request->getPost("agent_id");
        $view_data["show_description"] = $this->request->getPost("show_description");
    ...
    function validate_import_file_data($check_on_submit = false) {
        ...
        $row_count = 0;
        foreach ($excel_file_data as $ri => $r) {
            if ($ri === 0) {
                continue;
            }
            if (!array_filter($r)) {
        ...
        // check $max_import_rows
        $max_allowed = $this->max_import_rows;
        if ($row_count > $max_allowed) {
            if ($check_on_submit) {
                return false;
            } else {
                echo json_encode(array('success' => false, 'message' => sprintf(app_lang('import_max_row_limit_exceeded'), $max_allowed)));
                exit();
            }
        }
