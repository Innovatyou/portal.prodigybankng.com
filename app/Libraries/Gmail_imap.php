...
class Gmail_imap {
    ...
    private function _prepare_data($message_info) {
        ...
        // for gmail api, the first email has the same message id and thread id
        // for replying email, the message id is different but thread id is same
        // so we need to check if the thread id is already exists in the database
        $data["reference_id"] = $message_info->getThreadId();
