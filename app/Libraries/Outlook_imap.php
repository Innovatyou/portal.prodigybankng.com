...
class Outlook_imap {
    ...
    public function __construct() {
        ...
        $this->graph_url = "https://graph.microsoft.com/v1.0/me/";
    ...
    private function _prepare_data($message_info) {
        ...
        $data["reference_id"] = $message_info->conversationId;
    ...
    public function save_access_token($code, $is_refresh_token = false) {
        ...
        if ($is_refresh_token && !isset($result->refresh_token)) {
        ...
        $token_data = [
            ...
            "expires_at" => time() + $result->expires_in - 120 // the access token expires after 60 minutes, for buffering or any other issue save it 2 minutes less
        ];

        $new_access_token = encode_id(json_encode($token_data), "outlook_imap_oauth_access_token");
    ...
    private function do_request($method, $path = "", $body = array(), $decode_result = true) {
        ...
        // PREVENT expiry before request
        if (isset($oauth_access_token->expires_at) && time() >= $oauth_access_token->expires_at) {
