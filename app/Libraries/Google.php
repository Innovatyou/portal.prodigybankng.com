...
class Google {
    ...
    public function save_access_token($auth_code) {
        ...
        // check if parent folder id exists in database
        $existing_parent_folder_id = get_setting("google_drive_parent_folder_id");
        if ($existing_parent_folder_id) {
            $service = $this->_get_drive_service();
            if ($this->_does_folder_exist($service, $existing_parent_folder_id)) {
                // parent folder exists in drive, do nothing
                return;
            }
        }

        // either parent folder id doesn't exist in database or it doesn't exist in drive
    ...
    private function _get_temp_file_id($name = "") {
    ...
    private function _call_create_folder_action($service, $folder_name, $parent_folder_id = "") {
        $data = array(
            'name' => $folder_name,
        ...
        if ($parent_folder_id) {
            $data['parents'] = array($parent_folder_id);
        }
    ...
    private function _create_folder($folder_name = "", $path_type = "node", $parent_folder_id = null) {
        ...
        if ($path_type == "parent" || !$parent_folder_id) {

            //create parent folder with app title
            //in future, all uploads will be into this folder
            $drive_folder = $this->_call_create_folder_action($service, $folder_name);

            if (isset($drive_folder->id)) {
                $this->_save_id($folder_name, $drive_folder->id, "folder", $path_type);
                return $drive_folder->id;
            }

            log_message('error', 'Can not create parent folder to google drive.');
            exit();
        }

        // so it's a child folder
        // check if the folder id exists in the database
        $folder_id = $this->_get_saved_folder_id($folder_name);
        if ($folder_id) {
            return $folder_id;
        }

        // folder id doesn't found in the database, create it
        $drive_folder = $this->_call_create_folder_action($service, $folder_name, $parent_folder_id);
        if (isset($drive_folder->id)) {
            $this->_save_id($folder_name, $drive_folder->id, "folder", $path_type);
            return $drive_folder->id;
        }

        log_message('error', 'Can not create the folder ' . $folder_name . ' to google drive.');
        exit();
    }
    ...
    public function move_temp_file($file_name, $new_filename, $folder_name) {
        ...
        $fileId = $this->_get_temp_file_id($file_name);
        if (!$fileId) {
