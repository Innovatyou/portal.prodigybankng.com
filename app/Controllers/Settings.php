...
class Settings extends Security_Controller {
    ...
    function save_ticket_settings() {
        ...
        $settings = array("show_recent_ticket_comments_at_the_top", "ticket_prefix", "project_reference_in_tickets", "auto_close_ticket_after", "auto_reply_to_tickets", "auto_reply_to_tickets_message", "enable_embedded_form_to_get_tickets", "ask_for_confirmation_before_ticket_reply");
    ...
    function footer() {
        ...
        $footer_social_links = unserialize(get_setting("footer_social_links"));
        if ($footer_social_links && is_array($footer_social_links)) {
            foreach ($footer_social_links as $footer) {
                $footer_social_links_data .= $this->_make_footer_social_link_item_data(
                    $footer->social_link_type,
                    $footer->social_link_url,
                    isset($footer->social_link_type) ? $footer->social_link_type : "",
                    isset($footer->title) ? $footer->title : "",
                    isset($footer->icon_name) ? $footer->icon_name : ""
                );
            }
        }

        $view_data["footer_social_links"] = $footer_social_links_data;

        $footer_quick_links_data = "";
        $footer_quick_links = unserialize(get_setting("footer_quick_links"));
        if ($footer_quick_links && is_array($footer_quick_links)) {
            foreach ($footer_quick_links as $footer) {
                $footer_quick_links_data .= $this->_make_footer_quick_link_data($footer->link_name, $footer->link_url);
            }
        }

        $view_data["footer_quick_links"] = $footer_quick_links_data;
    ...
    private function _make_footer_social_link_item_data($social_link_type, $social_link_url, $type = "", $title = "", $icon_name = "") {
        ...
        if ($social_link_type === "custom") {
            $display_title = $title;
        } else {
            $display_title = $social_titles[$social_link_type] ?? ucfirst($social_link_type);
        }

        $edit = modal_anchor(get_uri("settings/footer_social_link_edit_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "float-end mr10 dark footer-social-link-edit-btn", "title" => app_lang('edit_footer_social_link'), "data-post-social_link_type" => $social_link_type, "data-post-social_link_url" => $social_link_url, "data-post-custom_social_title" => $title, "data-post-icon_name" => $icon_name));
        $delete = "<span class='footer-social-link-delete-btn ms-2 float-end clickable'><i data-feather='x' class='icon-16'></i></span>";
        $link = "<a href='$social_link_url' data-type='$social_link_type' data-icon_name='$icon_name' target='_blank'>$display_title</a>";

        $move_icon = "<div class='float-start move-icon'><i data-feather='menu' class='icon-16'></i></div>";

        if ($type == "data") {
            return $move_icon . $link . $delete . $edit;
        }

        return "<div class='list-group-item footer-social-link-item b-a rounded mb10' data-footer_social_link_temp_id='" . rand(2000, 400000000) . "'>"
            . $move_icon . $link . $delete . $edit . "</div>";
    }

    private function _make_footer_quick_link_data($quick_link_name, $quick_link_url, $type = "") {
        $edit = modal_anchor(get_uri("settings/footer_quick_link_edit_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "float-end mr10 dark footer-quick-link-edit-btn", "title" => app_lang('edit_footer_quick_link'), "data-post-quick_link_name" => $quick_link_name, "data-post-quick_link_url" => $quick_link_url));
        $delete = "<span class='footer-quick-link-delete-btn  float-end clickable'><i data-feather='x' class='icon-16'></i></span>";
        $title = "<a href='$quick_link_url' target='_blank'>$quick_link_name</a>";
        $move_icon = "<div class='float-start move-icon'><i data-feather='menu' class='icon-16'></i></div>";

        if ($type == "data") {
            return $move_icon . $title . $delete . $edit;
        } else {
            return "<div class='list-group-item footer-quick-link-item b-a rounded mb10' data-footer_quick_link_temp_id='" . rand(2000, 400000000) . "'>" . $move_icon . $title . $delete . $edit . "</div>";
        }
    }

    function save_footer_settings() {
        $settings = array("enable_footer", "footer_copyright_text", "footer_menus", "footer_brand_profile", "footer_about", "footer_social_links", "footer_quick_links_enabled", "footer_quick_links", "footer_custom_column", "footer_custom_column_title", "footer_custom_column_content", "footer_contact_info", "footer_address_1", "footer_address_2", "footer_email_1", "footer_email_2", "footer_phone_1", "footer_phone_2", "footer_contact_info_link", "footer_contact_info_link_title");
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting == "footer_quick_links") {
                $value = json_decode($value);
                $value = $value ? serialize($value) : serialize(array());
            }

            if ($setting == "footer_social_links") {
                $value = json_decode($value);
                $value = $value ? serialize($value) : serialize(array());
            }
    ...
    function footer_social_link_edit_modal_form() {
        $model_info = new \stdClass();
        $model_info->social_link_type = $this->request->getPost("social_link_type");
        $model_info->social_link_url = $this->request->getPost("social_link_url");
        $model_info->title = $this->request->getPost("custom_social_title");
        $model_info->icon_name = $this->request->getPost("icon_name");

        $view_data["model_info"] = $model_info;

        return $this->template->view("settings/footer/social_link_modal_form", $view_data);
    }

    function footer_quick_link_edit_modal_form() {
        $model_info = new \stdClass();
        $model_info->quick_link_name = $this->request->getPost("quick_link_name");
        $model_info->quick_link_url = $this->request->getPost("quick_link_url");

        $view_data["model_info"] = $model_info;

        return $this->template->view("settings/footer/quick_link_modal_form", $view_data);
    }
    ...
    function save_footer_quick_link() {
        $menu_name = $this->request->getPost("quick_link_name");
        $url = $this->request->getPost("quick_link_url");
        $type = $this->request->getPost("type");

        if ($menu_name && $url) {
            echo json_encode(array("success" => true, 'data' => $this->_make_footer_quick_link_data($menu_name, $url, $type)));
        }
    }

    function save_footer_social_link() {
        $social_type = $this->request->getPost("social_link_type");
        $link = $this->request->getPost("social_link");
        $title = $this->request->getPost("custom_social_title");
        $svg = $this->request->getPost("custom_social_svg");
        ...
        if ($social_type === "custom") {
            $uploaded_svg = $this->_upload_footer_social_icon();
            if ($uploaded_svg) {
                $svg = $uploaded_svg;
            }
            ...
            echo json_encode(array("success" => true, "data" => $this->_make_footer_social_link_item_data($social_type, $link, "default", $title, $svg)));
            return;
        }
        ...
        echo json_encode(array("success" => true, "data" => $this->_make_footer_social_link_item_data($social_type, $link, "default")));
    }

    private function _upload_footer_social_icon() {
        $posted_file_name = $this->request->getPost("custom_social_icon_file_name");
        $posted_file_content = $this->request->getPost("custom_social_icon_file_content");
        if ($posted_file_name && $posted_file_content) {
            return $this->_save_footer_social_icon($posted_file_name, $posted_file_content);
        }

        $file_data = get_array_value($_FILES, "icon_file");
        if (!($file_data && is_array($file_data) && count($file_data))) {
            return "";
        }
        ...
        return $this->_save_footer_social_icon($file_name, "", $temp_name);
    }

    private function _save_footer_social_icon($file_name, $file_content = "", $source_path = "") {
        $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        $allowed_icon_extensions = array("svg", "png", "jpg", "jpeg", "gif", "webp");
        if (!in_array($file_ext, $allowed_icon_extensions)) {
            return "";
        }

        $new_file_name = "footer-social-icon-" . uniqid() . "." . $file_ext;
        $target_path = get_setting("system_file_path") . "social/";

        $target_file = $target_path . $new_file_name;
        if ($file_content) {
            if (strpos($file_content, "base64,") !== false) {
        ...
        if (is_uploaded_file($source_path)) {
            return move_uploaded_file($source_path, $target_file) ? $new_file_name : "";
        }
    ...
    function save_contract_settings() {
        $settings = array("contract_prefix", "contract_color", "send_contract_bcc_to", "initial_number_of_the_contract", "add_signature_option_on_accepting_contract", "default_contract_template", "add_signature_option_for_team_members", "enable_contract_lock_state", "disable_contract_pdf_for_clients", "show_item_image_in_contract_item_list");
    ...
    function save_store_settings() {
        $settings = array("visitors_can_see_store_before_login", "payment_flow", "accept_order_before_login", "order_status_after_payment");

        $visitors_can_see_store_before_login = $this->request->getPost("visitors_can_see_store_before_login");
        $payment_flow = $this->request->getPost("payment_flow");
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "accept_order_before_login" && !($visitors_can_see_store_before_login && !$payment_flow)) {
                $value = "";
            }
    ...
    function chatgpt() {
        return $this->template->rander("settings/ai_assistant/chatgpt");
    }

    function save_chatgpt_settings() {
        $settings = array("enable_chatgpt", "chatgpt_api_key");
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting == "chatgpt_api_key") {
                $chatgpt_api_key = get_setting("chatgpt_api_key");

                if ($value === "******") {
                    $value = $chatgpt_api_key;
                } else {
                    $value = encode_id($value, "chatgpt_api_key");
                }

                //if user change the api key, flag chatgpt as unauthorized
                if (get_setting("chatgpt_authorized") && $chatgpt_api_key != $value) {
                    $this->Settings_model->save_setting("chatgpt_authorized", "0");
                }
            }
    ...
    function authorize_chatgpt() {
        // send a test message to check if the connection is successful
        $ChatGPT = new \App\Libraries\ChatGPT();
        ...
        $response = $ChatGPT->get_response($input_array);

        if ($response) {
            $this->Settings_model->save_setting("chatgpt_authorized", "1");
        } else {
            $this->Settings_model->save_setting("chatgpt_authorized", "0");
            $this->session->setFlashdata("error_message", app_lang("chatgpt_error_credentials_message"));
        }

        app_redirect("settings/chatgpt");
    }
    ...
    function ai_assistant_general() {
        return $this->template->rander("settings/ai_assistant/general");
    }

    function save_ai_assistant_general_settings() {
        $settings = array(
            "quick_assistant" => "quick_assistant_ai_agent",
            "ticket_assistant" => "ticket_assistant_ai_agents",
            "ai_chatbox" => "ai_chatbox_agents"
        );

        foreach ($settings as $key => $setting) {
            $value = $this->request->getPost($key);
            if (!$value) {
                // remove the corresponding setting
                $this->Settings_model->save_setting($setting, "");
            }
        }
    ...
    function quick_assistant_settings_modal_form() {
        $AI_agents_model = model("App\Models\AI_agents_model");
        $agents_dropdown = $AI_agents_model->get_id_and_text_dropdown(array("title"), array("status" => "active"));

        $view_data["agents_dropdown"] = json_encode($agents_dropdown);

        return $this->template->view("settings/ai_assistant/quick_assistant_settings_modal_form", $view_data);
    }

    function save_quick_assistant_settings() {

        $settings = array(
            "quick_assistant_ai_agent" => "quick_assistant_ai_agent",
            "ai_system_prompt_for_summarize" => "ai_system_prompt_for_summarize",
            "ai_system_prompt_for_improve" => "ai_system_prompt_for_improve",
            "ai_system_prompt_for_describe" => "ai_system_prompt_for_describe",
            "ai_system_prompt_for_key_points" => "ai_system_prompt_for_key_points"
        );
    ...
    function ticket_assistant_settings_modal_form() {
        $AI_agents_model = model("App\Models\AI_agents_model");
        $agents_dropdown = $AI_agents_model->get_id_and_text_dropdown(array("title"), array("status" => "active"));

        $view_data["agents_dropdown"] = json_encode($agents_dropdown);
        return $this->template->view("settings/ai_assistant/ticket_assistant_settings_modal_form", $view_data);
    }

    function save_ticket_assistant_settings() {
        $ticket_assistant_ai_agents = $this->request->getPost("ticket_assistant_ai_agents");
        $this->Settings_model->save_setting("ticket_assistant_ai_agents", $ticket_assistant_ai_agents);

        echo json_encode(array("success" => true, 'message' => app_lang('settings_updated')));
    }

    function ai_chatbox_settings_modal_form() {
        $AI_agents_model = model("App\Models\AI_agents_model");
        $agents_dropdown = $AI_agents_model->get_id_and_text_dropdown(array("title"), array("status" => "active"));

        $view_data["agents_dropdown"] = json_encode($agents_dropdown);
        return $this->template->view("settings/ai_assistant/ai_chatbox_settings_modal_form", $view_data);
    }

    function save_ai_chatbox_settings() {
        $ai_chatbox_agents = $this->request->getPost("ai_chatbox_agents");
        $this->Settings_model->save_setting("ai_chatbox_agents", $ai_chatbox_agents);

        echo json_encode(array("success" => true, 'message' => app_lang('settings_updated')));
    }

    function gemini() {
        return $this->template->rander("settings/ai_assistant/gemini");
    }

    function save_gemini_settings() {
        $settings = array("enable_gemini", "gemini_api_key");
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting == "gemini_api_key") {
                if ($value === "******") {
                    ...
                    $value = encode_id($value, "gemini_api_key");
                }

                // If user changes the API key, flag Gemini as unauthorized
                if (get_setting("gemini_authorized") && $gemini_api_key != $value) {
                    $this->Settings_model->save_setting("gemini_authorized", "0");
                }
            }
    ...
    function authorize_gemini() {

        // Send a test request to verify the API key
        $gemini = new \App\Libraries\Gemini();
        $response = $gemini->get_embedding("test");

        if ($response) {
            $this->Settings_model->save_setting("gemini_authorized", "1");
        } else {
            $this->Settings_model->save_setting("gemini_authorized", "0");
            $this->session->setFlashdata("error_message", app_lang("gemini_error_credentials_message"));
        }

        app_redirect("settings/gemini");
    }
}
