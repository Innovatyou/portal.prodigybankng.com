...
class Webhooks_listener extends App_Controller {
    ...
    private function subscription_payment_succeeded($payloads) {
        ...
        $stripe_subscription_id = $this->_get_stripe_subscription_id($payloads_data);
    ...
    private function _invoice_payment_succeeded($payloads) {
        ...
        //save the payment
        //not using Payment_processor here to prevent complexity
        ...
        if ($existing->id) {
            show_404(); // since it's webhook, nothing to redirect
        }
    ...
    private function _get_stripe_subscription_id($payloads_data) {
        $stripe_subscription_id = null;

        if (isset($payloads_data->parent->subscription_details->subscription)) {
            $stripe_subscription_id = $payloads_data->parent->subscription_details->subscription;
        } else if (isset($payloads_data->subscription)) {
