...
class Dashboard extends Security_Controller {
    ...
    private $client_dashboard_edit_from_admin = false;
    ...
    private function _check_widgets_for_staffs() {
        ...
        $show_project_timesheet = get_setting("module_project_timesheet");
        $show_ticket_timesheet = get_setting("module_ticket_timesheet");
        $show_general_task_timesheet = get_setting("module_general_task_timesheet");
        ...
        $show_timesheet = $show_project_timesheet || $show_ticket_timesheet || $show_general_task_timesheet;
        $show_lead = get_setting("module_lead");
        $show_contract = get_setting("module_contract");
        ...
        $access_contracts = $this->get_access_info("contract");
        ...
        if ($this->can_manage_all_projects() && !$this->permission_manager->has_all_projects_restricted_role()) {
        ...
        if (get_setting("module_project_timesheet") == "1" && ($this->login_user->is_admin || ($access_timesheets->access_type && !$this->permission_manager->has_all_projects_restricted_role()))) {
            $widget["total_project_hours"] = true;
        }

        if ($this->login_user->is_admin || (get_array_value($this->login_user->permissions, "can_manage_all_projects") === "1" && !$this->permission_manager->has_all_projects_restricted_role())) {
        ...
        if (!$this->has_all_projects_restricted_role()) {
            $widget["my_timesheet_statistics"] = $show_timesheet;
        }
        ...
        if ($show_contract && $access_contracts->access_type) {
            $widget["contracts_expiring_soon"] = true;
        }
    ...
    private function _check_widgets_for_clients() {
        ...
        if ($this->can_client_access("ticket") && $this->access_only_allowed_members_or_client_contact($this->login_user->client_id)) {
            $widget["new_tickets"] = true;
            $widget["open_tickets"] = true;
            $widget["closed_tickets"] = true;
            $widget["open_tickets_list"] = true;
    ...
    function view_default_widget($client_dashboard_edit_from_admin = 0) {
        $widget = $this->request->getPost("widget");
        if ($client_dashboard_edit_from_admin) {
            $this->client_dashboard_edit_from_admin = true;
    ...
    function save() {
        ...
        $data["data"] = $dashboard_data ? serialize($dashboard_data) : serialize(array());
    ...
    private function _widgets_row_data($widget_array, $view_type = "") {
        ...
        if (is_numeric($key)) {
            ...
            $client_dashboard_edit_from_admin = $this->client_dashboard_edit_from_admin ? 1 : 0;
            $details_button = modal_anchor(get_uri("dashboard/view_default_widget/" . $client_dashboard_edit_from_admin), "<i data-feather='more-horizontal' class='icon-16'></i>", array("class" => "text-off pr10 pl10", "title" => app_lang($key), "data-post-widget" => $key));
    ...
    private function _make_dashboard_widgets($widget = "") {
        ...
        if (is_numeric($widget)) {
            ...
            if ($this->login_user->user_type === "client" && $this->login_user->client_id) {
                ...
                $client_custom_fields = $this->Custom_field_values_model->get_details($custom_field_options)->getResult();
                ...
                foreach ($client_custom_fields as $field) {
                    ...
                    if ($field->value) {
                        $parser_data[$variable] = view("custom_fields/output_" . $field->custom_field_type, ["value" => $field->value]);
                    } else {
                        $parser_data[$variable] = "";
                    }

                    // Add template variable support
                    if ($field->template_variable_name) {
                ...
                if (!empty($matches[1])) {
                    foreach ($matches[1] as $variable) {
                        if (!array_key_exists($variable, $parser_data)) {
                            $parser_data[$variable] = "";
                        }
                    }
                }
                ...
                $view_data["widget_info"]->content = $content;
            }

            return $this->template->view("dashboards/custom_dashboards/extra_data/custom_widget", $view_data);
        }

        if ($this->client_dashboard_edit_from_admin) {
            $this->access_only_admin_or_settings_admin();
            $this->login_user->client_id = 0;
            $widgets_array = $this->_check_widgets_for_clients(); //just to show all widgets
    ...
    private function _get_widgets_for_staffs($widget, $widgets_array) {
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == "clock_in_out") {
                ...
                return get_invoices_value_widget("draft_count");
            ...
            } else if ($widget == "contracts_expiring_soon") {
                return contracts_expiring_soon_widget();
            }
