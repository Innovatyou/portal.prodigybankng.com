...
class Store extends Security_Controller {
    ...
    function place_order() {
        ...
        $options = array(
            "created_by" => (isset($this->login_user->id) ? $this->login_user->id : 0),
            "created_by_hash" => $this->get_cookie_hash(),
            "processing" => true
        );
        $order_items = $this->Order_items_model->get_details($options)->getResult();
        if (!$order_items) {
        ...
        if (isset($this->login_user->id)) {
            if ($this->login_user->user_type == "client") {
                ...
                if (get_setting("payment_flow") === "payment_order_invoice") {
                    $this->_process_payment_before_order($order_items);
                }
        ...
        $order_data = array_merge($order_data, get_next_display_id("order"));
        ...
        if ($order_id) {
            ...
            if (isset($this->login_user->id) && $this->login_user->user_type == "staff") {
                ...
                if (get_setting("payment_flow") === "order_invoice_payment") {
    ...
    private function _process_payment_before_order($order_items) {

        // save order info to verification table temporarily
        // check if there is any existing hash for the user
        // if exists, use that instead of creating a new one
        $existing_verification_id = 0;
        $order_payment_hash = get_setting("user_" . $this->login_user->id . "_order_payment_hash");
        if ($order_payment_hash) {

            $options = array("code" => $order_payment_hash, "type" => "invoice_payment");
            $verification_info = $this->Verification_model->get_details($options)->getRow();
            if ($verification_info && $verification_info->id) {
                $existing_verification_id = $verification_info->id;
            }
        }
        ...
        $verification_params = array(
            ...
            "client_id" => $this->login_user->client_id,
            "client_contact_id" => $this->login_user->id,
            "tax_id" => get_setting('order_tax_id') ? get_setting('order_tax_id') : 0,
            "tax_id2" => get_setting('order_tax_id2') ? get_setting('order_tax_id2') : 0,
            "created_by_hash" => $this->get_cookie_hash(),
            "files" => $files_data
        ...
        // save custom fields
        // store the post data to verification params
        $custom_field_values = array();
        $custom_fields = $this->Custom_fields_model->get_combined_details("orders", 0, 0, "client")->getResult();
        ...
        foreach ($custom_fields as $field) {
            ...
            if (array_key_exists($field_name, $_POST)) {
                $value = $this->request->getPost($field_name);
                $custom_field_values[$field_name] = $value;
            }
        }

        $verification_params['custom_field_values'] = $custom_field_values;

        $verification_data = array(
            "params" => serialize($verification_params)
        );

        if (!$existing_verification_id) {
            $code = make_random_string();
            $verification_data["type"] = "invoice_payment";
            $verification_data["code"] = $code;

            // save the code to settings
            $this->Settings_model->save_setting("user_" . $this->login_user->id . "_order_payment_hash", $code, "user");
        }

        $this->Verification_model->ci_save($verification_data, $existing_verification_id);

        // delete the order items (flag as delete)
        // we'll revert it back when the payment is completed
        foreach ($order_items as $order_item) {
            $this->Order_items_model->delete($order_item->id);
        }

        echo json_encode(array("success" => true, "redirect_to" => get_uri("store/process_payment"), 'message' => app_lang('record_saved')));
        exit;
    }
    ...
    function process_payment() {
        ...
        // check client id and client contact id
        if ($this->login_user->client_id !== get_array_value($verification_data, "client_id") || $this->login_user->id !== get_array_value($verification_data, "client_contact_id")) {
            show_404();
        }

        // create a virtual invoice
        // get an empty invoice info class from database
        $invoice_info = $this->Invoices_model->get_one_where(array('id' => 0));
        $invoice_info->client_id = get_array_value($verification_data, "client_id");
        $invoice_id = 0;

        $view_data['invoice_info'] = $invoice_info;
        $view_data['invoice_id'] = $invoice_id;
        $view_data['contact_user_id'] = get_array_value($verification_data, "client_contact_id");;
        $view_data['verification_code'] = $order_payment_hash;
        $view_data["invoice_total_summary"] = $this->Orders_model->get_order_total_summary_from_verification_data($verification_data);
        $view_data['payment_methods'] = $this->Payment_methods_model->get_available_online_payment_methods();
        $view_data['client_info'] = $this->Clients_model->get_one($invoice_info->client_id);

        $paytm = new Paytm();
        $view_data['paytm_url'] = $paytm->get_paytm_url();

        return $this->template->rander("invoices/pay_without_invoice", $view_data);
    }
}
