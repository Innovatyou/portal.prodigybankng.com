...
class Proposals extends Security_Controller {
    ...
    function save() {
        ...
        if ($id) {
            if ($this->login_user->is_admin) {
                $proposal_data["display_id"] = $this->request->getPost('display_id');
            }
        }
        ...
        if (!$id) {
            ...
            $proposal_data = array_merge($proposal_data, get_next_display_id("proposal"));
        }
    ...
    private function _make_row($data, $custom_fields, $is_mobile = 0) {
        ...
        if ($this->login_user->user_type == "staff" && $can_manage_proposals) {
            $proposal_url = anchor(get_uri("proposals/view/" . $data->id), $data->display_id);
        } else {
            //for client client
            $proposal_url = anchor(get_uri("proposals/preview/" . $data->id), $data->display_id);
        }
    ...
    private function _make_item_row($data) {
        $is_editable = $this->_is_proposal_editable($data->proposal_id);
        $show_item_image = get_setting("show_item_image_in_proposal_item_list");

        $move_icon = $is_editable ? "<div class='float-start move-icon'><i data-feather='menu' class='icon-16'></i></div>" : "";
    ...
    function view_pdf($proposal_id) {
        ...
        $view_data["pdf_path"] = get_uri("proposals/download_pdf/" . $proposal_id . "/view");
