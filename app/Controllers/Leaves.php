...
class Leaves extends Security_Controller {
    ...
    function application_details() {
        ...
        $info = $this->Leave_applications_model->get_details_info($application_id);
        if (!$info) {
            show_404();
        }
    ...
    function update_status() {
        ...
        $application_info = $this->Leave_applications_model->get_one($application_id);

        if ($status === "approved" || $status === "rejected") {
            $this->access_only_allowed_members($application_info->applicant_id);
        } else if ($status === "canceled" && $application_info->applicant_id != $this->login_user->id) {
            //any user can't cancel other user's leave application
            app_redirect("forbidden");
        }

        //user can update only the applications where status = pending
        if ($application_info->status != "pending" || !($status === "approved" || $status === "rejected" || $status === "canceled")) {
            app_redirect("forbidden");
        }

        //if status is rejected, then rejection_reason is required
        if ($status === "rejected") {
            $leave_data['rejection_reason'] = $this->request->getPost('rejection_reason');
        }

        $save_id = $this->Leave_applications_model->ci_save($leave_data, $application_id);
        if ($save_id) {

            $notification_options = array("leave_id" => $application_id, "to_user_id" => $application_info->applicant_id);
    ...
    function delete() {
        ...
        $application_info = $this->Leave_applications_model->get_one($id);
        $this->access_only_allowed_members($application_info->applicant_id);
    ...
    function reject_reason_modal_form($application_id) {
        validate_numeric_value($application_id);

        $application_info = $this->Leave_applications_model->get_one($application_id);
        $this->access_only_allowed_members($application_info->applicant_id);

        $view_data["leave_id"] = $application_id;

        return $this->template->view('leaves/reject_reason_modal_form', $view_data);
    }
}
