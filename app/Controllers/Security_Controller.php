...
class Security_Controller extends App_Controller {
    ...
    protected function _make_order_item_row($data) {
        ...
        $edit_option = "";
        $delete_option = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("store/delete_item"), "data-action" => "delete"));
        if (isset($this->login_user->user_type) && $this->login_user->user_type == "staff") {
            $edit_option = modal_anchor(get_uri("store/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id, "data-post-order_id" => $data->order_id));
        }
    ...
    protected function init_project_settings($project_id) {
        ...
        $project_settings_model = model('App\Models\Project_settings_model');
        $settings = $project_settings_model->get_all_where(array("project_id" => $project_id))->getResult();
    ...
    protected function can_view_timesheet($project_id = 0, $show_all_personal_timesheets = false) {
        ...
        $ticket_timesheet = get_setting("module_ticket_timesheet");
        $general_task_timesheet = get_setting("module_general_task_timesheet");

        if (($project_id && !$project_timesheet) || (!$project_id && !$project_timesheet && !$ticket_timesheet && !$general_task_timesheet)) {
            return false;
        }
    ...
    protected function _get_project_info_data($project_id) {
        $options = array(
            "id" => $project_id,
        ...
        if ($project_info) {
            $view_data['project_info'] = $project_info;
            $timer = $this->Timesheets_model->get_timer_info("project", $project_id, $this->login_user->id)->getRow();
            $user_has_any_timer_except_this_project = $this->Timesheets_model->user_has_any_timer_except_this_context("project", $project_id, $this->login_user->id);
