...
class Expenses extends Security_Controller {
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $file_icon = get_file_icon(strtolower(pathinfo($file_name, PATHINFO_EXTENSION)), false);
                    ...
                    $files_link .= js_anchor($file_icon, array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "float-start mr10", "title" => remove_file_prefix($file_name), "data-url" => get_uri("expenses/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }

        $files = "<div class='d-flex align-items-center'>" . $files_link . $file_download_link . "</div>";
    ...
    function income_vs_expenses_chart_data() {
        ...
        if ($year) {
            if (!$currency ||  (get_setting("default_currency") == $currency)) {
