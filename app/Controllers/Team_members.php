...
class Team_members extends Security_Controller {
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                if ((get_setting("module_project_timesheet") == "1" || get_setting("module_ticket_timesheet") == "1" || get_setting("module_general_task_timesheet") == "1") && ($this->login_user->is_admin || ($this->login_user->id === $user_info->id) || ($access_timesheets->access_type && !$this->has_all_projects_restricted_role()))) {
    ...
    function save_account_settings($user_id) {
        ...
        if ($password) {
            if ($this->is_own_id($user_id) || $this->login_user->is_admin) {
    ...
    private function _make_file_row($data) {
        $file_icon = get_file_icon(strtolower(pathinfo($data->file_name, PATHINFO_EXTENSION)), false);
