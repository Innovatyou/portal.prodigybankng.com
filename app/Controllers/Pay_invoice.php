...
class Pay_invoice extends Security_Controller {
    ...
    function init_online_payment() {

        $login_user_id = isset($this->login_user->id) ? $this->login_user->id : 0;

        if ($login_user_id) {
            // logged in user, should be client
            $this->access_only_clients();
        }

        if (!$login_user_id && !get_setting("client_can_pay_invoice_without_login")) {
            // not logged in user, the setting should be enabled
            app_redirect("forbidden");
        }

        $Payment_processor = new Payment_processor($this);
        try {
            $checkout_url = $Payment_processor->init_online_payment($login_user_id);
            if ($checkout_url) {
