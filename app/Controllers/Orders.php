...
class Orders extends Security_Controller {
    ...
    private function _make_row($data, $custom_fields, $is_mobile = 0) {
        $order_url = "";
        if ($this->login_user->user_type == "staff") {
            $order_url = anchor(get_uri("orders/view/" . $data->id), $data->display_id);
        } else {
            //for client
            $order_url = anchor(get_uri("store/order_preview/" . $data->id), $data->display_id);
        }
    ...
    function save() {
        ...
        if ($id) {
            ...
            if ($this->login_user->is_admin) {
                $order_data["display_id"] = $this->request->getPost('display_id');
            }
        }
    ...
    function view_pdf($order_id) {
        validate_numeric_value($order_id);

        $this->access_only_allowed_members();

        $view_data["pdf_path"] = get_uri("orders/download_pdf/" . $order_id . "/view");
