...
class Roles extends Security_Controller {
    ...
    function save_permissions() {
        ...
        $hide_team_members_list_from_dropdowns = $this->request->getPost('hide_team_members_list_from_dropdowns');
        ...
        $can_access_ai_chatbox = $this->request->getPost('can_access_ai_chatbox');
        $accessible_ai_agents_permission = $this->request->getPost('accessible_ai_agents_permission');
        $accessible_ai_agents_permission_specific = $this->request->getPost('accessible_ai_agents_permission_specific');
        ...
        $permissions = array(
            ...
            "hide_team_members_list_from_dropdowns" => $hide_team_members_list_from_dropdowns,
            ...
            "accessible_ai_agents_permission" => $accessible_ai_agents_permission,
            "accessible_ai_agents_permission_specific" => $accessible_ai_agents_permission_specific,
        );
    ...
    private function _make_user_role_row($data) {
        ...
        if ($this->login_user->id !== $data->id) {
            $edit = modal_anchor(get_uri("roles/user_role_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_user_role'), "data-post-id" => $data->id));
        }
    ...
    function user_role_modal_form() {
        ...
        $id = $this->request->getPost('id');
        $view_data["team_members_info"] = $this->Users_model->get_one($id);
    ...
    function save_user_role() {
        ...
        if ($save_id) {
            echo json_encode(array("success" => true, "data" => $this->_user_role_row_data($save_id), 'id' => $save_id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }
