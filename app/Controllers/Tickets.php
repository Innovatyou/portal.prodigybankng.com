...
class Tickets extends Security_Controller {
    ...
    function save() {
        ...
        if (!$id) {
            ...
            $ticket_data = array_merge($ticket_data, get_next_display_id("ticket"));
        }
        ...
        if ($id) {
            ...
            if ($this->login_user->is_admin) {
                $ticket_data["display_id"] = $this->request->getPost('display_id');
            }
        }
    ...
    private function _make_row($data, $custom_fields, $is_mobile = 0, $view_type = "", $selected_client_id = 0) {
        ...
        $row_data = array(
            $status_color,
            $data->id,
            anchor(get_uri("tickets/view/" . $data->id), $data->display_id, array("class" => "js-selection-id", "data-id" => $data->id, "title" => "")),
            $title,
    ...
    function view($ticket_id = 0, $client_id = 0) {
        ...
        if ($ticket_id) {
            ...
            if ($ticket_info) {
                ...
                $timer = $this->Timesheets_model->get_timer_info("ticket", $ticket_id, $this->login_user->id)->getRow();
                $user_has_any_timer_except_this_ticket = $this->Timesheets_model->user_has_any_timer_except_this_context("ticket", $ticket_id, $this->login_user->id);

                //disable the start timer button if the setting is disabled
                $view_data["disable_timer"] = false;
                if ($user_has_any_timer_except_this_ticket && !get_setting("users_can_start_multiple_timers_at_a_time")) {
                ...
                $ticket_assistant_ai_agents = $this->_get_ticket_assistant_ai_agents();
                $view_data["ticket_assistant_ai_agents"] = $ticket_assistant_ai_agents;

                $view_data["show_ticket_timer"] = get_setting("module_ticket_timesheet") ? true : false;

                //count total worked hours in a ticket
                $timesheet_options = array("context" => "ticket", "context_id" => $ticket_id);

                if (!$this->login_user->is_admin) {
                    $timesheet_options["user_id"] = $this->login_user->id;
                }

                $info = $this->Timesheets_model->count_total_time($timesheet_options);
                $view_data["total_ticket_hours"] = convert_seconds_to_time_format($info->timesheet_total);
    ...
    //get accessible ai agents
    private function _get_ticket_assistant_ai_agents() {

        $ticket_assistant_ai_agents = get_setting("ticket_assistant_ai_agents");
        if (!$ticket_assistant_ai_agents) {
            return array();
        }

        $ticket_assistant_ai_agents = explode(",", $ticket_assistant_ai_agents);

        foreach ($ticket_assistant_ai_agents as $key => $agent_id) {
            if (!$this->permission_manager->can_access_this_ai_agent($agent_id)) {
                unset($ticket_assistant_ai_agents[$key]);
            }
        }
        ...
        return $this->AI_agents_model->get_details(array("agent_ids" => implode(",", $ticket_assistant_ai_agents)))->getResult();
    }
    ...
    function change_email_modal_form($ticket_id = 0) {
        $this->access_only_team_members();
        $this->validate_ticket_access($ticket_id);

        $view_data["ticket_info"] = $this->Tickets_model->get_one($ticket_id);
        return $this->template->view("tickets/change_email_modal_form", $view_data);
    }
    ...
    function change_email_address() {
        ...
        if ($this->Users_model->is_email_exists($email, 0, true)) {
    ...
    function delete_selected_tickets() {
        ...
        if ($ticket_ids) {
            ...
            foreach ($ticket_ids_array as $id) {
                validate_numeric_value($id);
                $this->validate_ticket_access($id);
