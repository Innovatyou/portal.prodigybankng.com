...
class Leads extends Security_Controller {
    ...
    private function _make_file_row($data) {
        ...
        $file = get_file_icon($file_ext, false) . $description;
    ...
    private function _get_kanban_data($options) {
        return $this->Clients_model->get_leads_kanban_details($options)->getResult();
    }

    // Used by Kanban_view trait
    private function _get_kanban_statuses() {
        return $this->Lead_status_model->get_details();
    }

    // Used by Kanban_view trait
    private function _get_kanban_counts($extra_data = []) {
        $lead_count_query_options = $extra_data;
        $lead_count_query_options["return_lead_counts_only"] = true;
        $lead_counts = $this->Clients_model->get_leads_kanban_details($lead_count_query_options)->getResult();

        $column_lead_count = array();
        foreach ($lead_counts as $lead_count) {
            $column_lead_count[$lead_count->lead_status_id] = $lead_count->leads_count;
        }

        return $column_lead_count;
    }
    ...
    function all_leads_kanban_data() {
        ...
        return $this->_render_kanban_view($extra_data);
    }

    protected function _kanban_extra_view_data($items, $status_id = 0, $options = []) {
        return [
            "kanban_permissions" => [
                "can_drag_items" => true
            ]
