...
class Contracts extends Security_Controller {
    ...
    function save() {
        ...
        if ($id) {
            ...
            if ($this->login_user->is_admin) {
                $contract_data["display_id"] = $this->request->getPost('display_id');
            }
        }
        ...
        if (!$id) {
            ...
            $contract_data = array_merge($contract_data, get_next_display_id("contract"));
        }
    ...
    function list_data($is_mobile = 0, $is_widget = 0) {
    ...
    private function _make_row($data, $custom_fields, $is_mobile = 0) {
        ...
        if ($this->can_edit_contracts()) {
            $contract_id = anchor(get_uri("contracts/view/" . $data->id), $data->display_id);
        } else {
            //for client
            $contract_id = anchor(get_uri("contracts/preview/" . $data->id), $data->display_id);
        }
    ...
    private function _make_item_row($data) {
        $is_editable = $this->_is_contract_editable($data->contract_id);
        $show_item_image = get_setting("show_item_image_in_contract_item_list");

        $move_icon = $is_editable ? "<div class='move-icon'><i data-feather='menu' class='icon-16'></i></div>" : "";

        $desc_style = ($is_editable && !$show_item_image) ? "style='margin-left:30px'" : "";

        $item_image = "";
        if ($show_item_image) {
            $item_image = "<div class='d-flex align-items-center'>$move_icon<img src='" . get_store_item_image($data->files) . "' class='item-list-image' /></div>";
        }

        $item_title_prefix = $show_item_image ? "" : $move_icon;

        $item = "<div class='item-row strong mb5' data-id='{$data->id}'>$item_title_prefix {$data->title} </div>";
    ...
    function view_pdf($contract_id) {
        ...
        $view_data["pdf_path"] = get_uri("contracts/download_pdf/" . $contract_id . "/view");
