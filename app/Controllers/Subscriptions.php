...
class Subscriptions extends Security_Controller {
    ...
    function save() {
        ...
        $subscription_data = array_merge($subscription_data, get_next_display_id("subscription"));
        ...
        if ($id) {
            ...
            if ($this->login_user->is_admin) {
                $subscription_data["display_id"] = $this->request->getPost('display_id');
            }
        }
    ...
    private function _make_row($data, $custom_fields, $is_mobile = 0) {
        ...
        if ($this->login_user->user_type == "staff") {
            $subscription_url = anchor(get_uri("subscriptions/view/" . $data->id), $data->display_id);
            $subscription_title = anchor(get_uri("subscriptions/view/" . $data->id), $data->title);
        } else {
            $subscription_url = anchor(get_uri("subscriptions/preview/" . $data->id), $data->display_id);
    ...
    function save_item() {
        ...
        if ($subscription_item_id) {
            ...
            echo json_encode(array("success" => true, "subscription_id" => $item_info->subscription_id, "data" => $this->_make_item_row($item_info, $subscription_info->type), 'id' => $subscription_item_id, 'subscription_type' => $subscription_info->type, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }

    /* delete or undo an subscription item */
    ...
    function delete_item() {
        ...
        $id = $this->request->getPost('id');
        $item_info = $this->Subscription_items_model->get_one($id);
        $subscription_info = $this->Subscriptions_model->get_one($item_info->subscription_id);

        $this->validate_subscription_access($item_info->subscription_id);

        if ($this->request->getPost('undo')) {
            if ($this->Subscription_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Subscription_items_model->get_details($options)->getRow();
                echo json_encode(array("success" => true, "subscription_id" => $item_info->subscription_id, "data" => $this->_make_item_row($item_info, $subscription_info->type), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Subscription_items_model->delete($id)) {
                $item_info = $this->Subscription_items_model->get_one($id);
                echo json_encode(array("success" => true, "subscription_id" => $item_info->subscription_id, 'message' => app_lang('record_deleted')));
            } else {
                echo json_encode(array("success" => false, 'message' => app_lang('record_cannot_be_deleted')));
            }
        }
    ...
    function item_list_data($subscription_id = 0, $subscription_type = "") {
        validate_numeric_value($subscription_id);
        if (!$this->can_view_subscriptions()) {
            app_redirect("forbidden");
        }
        ...
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_item_row($data, $subscription_type);
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_item_row($data, $subscription_type = "") {
        ...
        $delete_btn = "";
        if ($subscription_type == "app") {
            $delete_btn = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("subscriptions/delete_item"), "data-action" => "delete"));
        }
        ...
        return array(
            ...
            modal_anchor(get_uri("subscriptions/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_subscription'), "data-post-id" => $data->id)) . $delete_btn
        );
    }
