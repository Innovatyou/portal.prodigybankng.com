...
class Users_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $exclude_client_id = $this->_get_clean_value($options, "exclude_client_id");
        if ($exclude_client_id) {
            $where .= " AND $users_table.client_id!=$exclude_client_id";
        }
    ...
    function is_email_exists($email, $target_client_id = 0, $only_staff = false) {
        $users_table = $this->db->prefixTable('users');
        ...
        $user_type_condition = $only_staff ? " $users_table.user_type='staff'" : " $users_table.user_type IN ('staff','client')";

        $sql = "SELECT $users_table.id, $users_table.user_type, $users_table.client_id
                FROM $users_table   
                WHERE $users_table.deleted=0 AND $users_table.email='$email' AND $user_type_condition ";
    ...
    function get_client_contacts_except($client_id) {
        $users_table = $this->db->prefixTable('users');
        ...
        INNER JOIN (
            SELECT $users_table.email, MIN($users_table.id) AS min_id
            FROM $users_table
            WHERE $users_table.deleted = 0 AND $users_table.user_type = 'client' $where
            GROUP BY $users_table.email
        ) AS min_users
        ON users.id = min_users.min_id
        ORDER BY users.first_name, users.last_name";

        return $this->db->query($sql);
    }

    function is_team_member_email_exists($email) {
        return $this->is_email_exists($email, 0, true);
    }
}
