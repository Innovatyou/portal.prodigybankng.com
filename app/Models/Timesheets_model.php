...
class Timesheets_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($project_id) {
            $where .= " AND $timesheet_table.context_id=$project_id AND $timesheet_table.context='project'";
        }
        ...
        if ($client_id) {
            $where .= " AND $timesheet_table.context_id IN(SELECT $projects_table.id FROM $projects_table WHERE $projects_table.client_id=$client_id) AND $timesheet_table.context='project'";
        }
        ...
        if ($context) {
            $where .= " AND $timesheet_table.context = '$context'";
        }

        $context_id = get_array_value($options, "context_id");
        if ($context_id) {
            $where .= " AND $timesheet_table.context_id = '$context_id'";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $timesheet_table.*,  CONCAT($users_table.first_name, ' ',$users_table.last_name) AS logged_by_user, $users_table.image as logged_by_avatar,
            ...
            $tickets_table.title AS ticket_title $select_custom_fieds
        ...
        LEFT JOIN $projects_table ON $projects_table.id= $timesheet_table.context_id AND $timesheet_table.context= 'project'
        LEFT JOIN $tickets_table ON $tickets_table.id= $timesheet_table.context_id AND $timesheet_table.context= 'ticket'
        $join_custom_fieds
    ...
    function get_summary_details($options = array()) {
        ...
        $client_id = $this->_get_clean_value($options, "client_id");
        if ($client_id) {
            $where .= " AND $timesheet_table.context_id IN(SELECT $projects_table.id FROM $projects_table WHERE $projects_table.client_id=$client_id AND $projects_table.context='project')";
        }
        ...
        //group by
        $group_by_option = "$timesheet_table.user_id, $timesheet_table.task_id, $timesheet_table.context_id";
        ...
        if ($group_by === "member") {
            ...
            $group_by_option = "$timesheet_table.context_id";
        }
        ...
        $sql = "SELECT new_summary_table.user_id, new_summary_table.total_duration, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS logged_by_user, $users_table.image as logged_by_avatar,
                       ...
                       $projects_table.client_id AS timesheet_client_id, (SELECT $clients_table.company_name FROM $clients_table WHERE $clients_table.id=$projects_table.client_id AND $clients_table.deleted=0) AS timesheet_client_company_name, new_summary_table.context
                FROM (SELECT MAX($timesheet_table.context_id) AS context_id, MAX($timesheet_table.user_id) AS user_id, MAX($timesheet_table.task_id) AS task_id, (SUM(TIMESTAMPDIFF(SECOND, $timesheet_table.start_time, $timesheet_table.end_time)) + SUM(ROUND(($timesheet_table.hours * 60), 0) * 60)) AS total_duration, $timesheet_table.context
                        FROM $timesheet_table
                        WHERE $timesheet_table.deleted=0 $where $custom_fields_where
                ...
                LEFT JOIN $projects_table ON $projects_table.id= new_summary_table.context_id AND new_summary_table.context='project'            
                ";
        return $this->db->query($sql);
    }

    function get_timer_info($context, $context_id, $user_id) {
        $where = array("context" => $context, "context_id" => $context_id, "user_id" => $user_id, "status" => "open", "deleted" => 0);
        return parent::get_all_where($where);
    }
    ...
    function process_timer($data) {
        ...
        $context = $this->_get_clean_value($data, "context");
        $context_id = $this->_get_clean_value($data, "context_id");
        $user_id = $this->_get_clean_value($data, "user_id");
        $note = $this->_get_clean_value($data, "note");
        $task_id = $this->_get_clean_value($data, "task_id");

        // Base condition: one open timer per user per context
        $where = array(
            "context" => $context,
        ...
        // If task exists, check task-specific timer first
        if ($task_id) {
    ...
    function get_open_timers($user_id = 0) {
        ...
        $sql = "SELECT $timesheet_table.*,
                $projects_table.title AS project_title,
                $tasks_table.title AS task_title,
                $tickets_table.title AS ticket_title
            FROM $timesheet_table
            LEFT JOIN $projects_table ON $timesheet_table.context = 'project' AND $projects_table.id = $timesheet_table.context_id
            LEFT JOIN $tickets_table ON $timesheet_table.context = 'ticket' AND $tickets_table.id = $timesheet_table.context_id
            LEFT JOIN $tasks_table ON $tasks_table.id = $timesheet_table.task_id
    ...
    function active_members_on_projects() {
        ...
        $sql = "SELECT CONCAT($users_table.first_name, ' ',$users_table.last_name) AS member_name, $users_table.image, $users_table.id,
                    (SELECT GROUP_CONCAT($project_time_table.context_id, '--::--', $projects_table.title, '--::--', $project_time_table.start_time)
                    FROM $project_time_table 
                    LEFT JOIN $projects_table ON $projects_table.id = $project_time_table.context_id AND $project_time_table.context='project'
    ...
    function user_has_any_timer_except_this_context($context, $context_id, $user_id) {
        $timesheet_table = $this->db->prefixTable('project_time');
        ...
        $sql = "SELECT COUNT($timesheet_table.id) AS total_timers
            FROM $timesheet_table
            WHERE $timesheet_table.deleted = 0
              AND $timesheet_table.user_id = $user_id
              AND $timesheet_table.status = 'open'
              AND NOT ($timesheet_table.context = '$context' AND $timesheet_table.context_id = $context_id)";

        return (int) $this->db->query($sql)->getRow()->total_timers;
    }
    ...
    function count_total_time($options = array()) {
        ...
        $allowed_members = $this->_get_clean_value($options, "allowed_members");
        if ($project_id) {
            $timesheet_where .= " AND $timesheet_table.context_id=$project_id AND $timesheet_table.context='project'";
        ...
        $context = get_array_value($options, "context");
        if ($context) {
            $timesheet_where .= " AND $timesheet_table.context = '$context'";
        }

        $context_id = get_array_value($options, "context_id");
        if ($context_id) {
            $timesheet_where .= " AND $timesheet_table.context_id = '$context_id'";
        }
    ...
    private function get_timesheet_own_project_memeber_only_query($timesheet_table) {
        ...
        if ($login_user_id) {
            ...
            return " AND $timesheet_table.context_id IN(SELECT $project_members_table.project_id FROM $project_members_table WHERE $project_members_table.deleted=0 AND $project_members_table.user_id=$login_user_id) AND $timesheet_table.context='project'";
        }
    }
