...
class Notifications_model extends Crud_model {
    ...
    function get_notifications($user_id, $offset = 0, $limit = 20) {
        ...
        $sql = "SELECT 
            ...
            $contracts_table.meta_data AS contract_meta_data,
            ...
            COALESCE(ev_main.title, ev_reminder.title) AS event_title,
            ...
            lead_clients.company_name AS lead_company_name,
            ...
            invoice_clients.currency_symbol AS client_currency_symbol,
            payment_invoice_table.display_id AS payment_invoice_display_id,

            CONCAT(to_users.first_name, ' ', to_users.last_name) AS to_user_name,

            FIND_IN_SET($user_id, $notifications_table.read_by) AS is_read,

            $orders_table.display_id AS order_display_id,
            $estimates_table.display_id AS estimate_display_id,
            $proposals_table.display_id AS proposal_display_id,
            $contracts_table.display_id AS contract_display_id,
            $subscriptions_table.display_id AS subscription_display_id,
            $tickets_table.display_id AS ticket_display_id

        FROM $notifications_table
        LEFT JOIN $users_table AS u ON u.id = $notifications_table.user_id
        LEFT JOIN $users_table AS to_users ON to_users.id = $notifications_table.to_user_id
        ...
        LEFT JOIN $invoices_table ON $invoices_table.id = $invoice_payments_table.invoice_id AND $invoices_table.deleted = 0
        LEFT JOIN $clients_table AS invoice_clients ON invoice_clients.id = $invoices_table.client_id
        LEFT JOIN $clients_table ON $clients_table.id = $notifications_table.client_id
        LEFT JOIN $clients_table AS lead_clients ON lead_clients.id = $notifications_table.lead_id
        ...
        LEFT JOIN $events_table AS ev_main ON ev_main.id = $notifications_table.event_id
        LEFT JOIN $events_table AS ev_reminder ON ev_reminder.id = $notifications_table.reminder_id
        LEFT JOIN $orders_table ON $orders_table.id = $notifications_table.order_id
        LEFT JOIN ( SELECT id, display_id 
            FROM $invoices_table 
            WHERE deleted = 0
        ...
        $found_rows = $query->getNumRows();

        if ($found_rows == $limit) {
            $found_rows = $offset + $found_rows + 1; //based on best guess to reduce the data call
        }
