...
class Projects_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($title) {
            $where .= " AND $projects_table.title='$title'";
        }
        ...
        LEFT JOIN $orders_table ON $orders_table.project_id = $projects_table.id
        LEFT JOIN $estimates_table ON $estimates_table.id = $projects_table.estimate_id
        LEFT JOIN $proposals_table ON $proposals_table.id = $projects_table.proposal_id
        $extra_join   
        $join_custom_fieds    
    ...
    function get_team_members_summary($options = array()) {
        ...
        $sql = "SELECT  $users_table.id as team_member_id, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS team_member_name, $users_table.image, 
                ...
                LEFT JOIN (
                    SELECT SUM(TIME_TO_SEC(TIMEDIFF($select_tz_end_time,$select_tz_start_time))) 
                    + SUM(ROUND(($timesheet_table.hours * 60), 0) * 60) AS total_secconds_worked,
                    $timesheet_table.context_id AS project_id,
                    $timesheet_table.user_id 
                FROM $timesheet_table 
                WHERE $timesheet_table.deleted=0 
                AND $timesheet_table.context = 'project'
                GROUP BY $timesheet_table.context_id, $timesheet_table.user_id 
                ) AS timesheet_table 
                ON timesheet_table.project_id = $project_members_table.project_id 
                AND timesheet_table.user_id = $project_members_table.user_id 
    ...
    function get_clients_summary($options = array()) {
        ...
        $sql = "SELECT $clients_table.id AS client_id, $clients_table.company_name AS client_name,
                ...
                LEFT JOIN (SELECT SUM(TIME_TO_SEC(TIMEDIFF($select_tz_end_time, $select_tz_start_time))) + SUM(ROUND(($timesheet_table.hours * 60), 0) * 60) AS total_secconds_worked, $timesheet_table.context_id AS project_id FROM $timesheet_table WHERE $timesheet_table.deleted = 0 AND $timesheet_table.context = 'project' GROUP BY $timesheet_table.context_id
