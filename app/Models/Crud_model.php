...
class Crud_model extends Model {
    ...
    protected function _get_clean_value($options_or_value, $key = "", $escape = true) {
        ...
        if (is_string($value)) {
            ...
            //     throw new \RuntimeException('Input is too long detected by _get_clean_value where the key');
            // }
            ...
            if (($length === 10 && preg_match('/^\d{4}-\d{2}-\d{2}$/', $value))
                ...
                if (!$d || $d->format($date_format) !== $value) {
                    ...
                    throw new \RuntimeException('Invalid date detected by _get_clean_value where the key is ' . $key);
                }
            ...
            if (preg_match('/\b(ASCII|SUBSTRING|MID|LENGTH|DATABASE|SCHEMA|BENCHMARK|SLEEP|VERSION|CHAR|CONCAT)\b/i', $value)) {
                ...
                throw new \RuntimeException('SQL function injection detected by _get_clean_value where the key is ' . $key);
            }

            // Protect against common SQL keywords, harmful characters, and patterns
            if (
                preg_match('/(?<![A-Za-z0-9_-])(TABLE|UNION(?:\s+ALL)?|INSERT|DELETE|UPDATE|EXEC|DROP|ALTER|TRUNCATE|REPLACE|LOAD_FILE|OUTFILE|INTO|GROUP\s+BY|ORDER\s+BY|HAVING|CASE|LIKE|--|#|\/\*)(?![A-Za-z0-9_-])/i', $value)
                ...
                throw new \RuntimeException('Harmful injection detected by _get_clean_value where the key is ' . $key);
            }
    ...
    protected function _get_clean_id($id, $required = false) {
        ...
        if ($required && $id <= 0) {
            ...
            throw new \RuntimeException('Invalid id detected in _get_clean_id where the id is ' . $id);
        }
