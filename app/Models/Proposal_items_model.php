...
class Proposal_items_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $proposal_items_table.*, (SELECT $clients_table.currency_symbol FROM $clients_table WHERE $clients_table.id=$proposals_table.client_id limit 1) AS currency_symbol, $items_table.files
        FROM $proposal_items_table
        LEFT JOIN $proposals_table ON $proposals_table.id=$proposal_items_table.proposal_id
        LEFT JOIN $items_table ON $items_table.id=$proposal_items_table.item_id
