...
class Tasks_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $exclude_contexts = $this->_get_clean_value($options, "exclude_contexts");
        if ($exclude_contexts) {
            if (is_array($exclude_contexts)) {
                $exclude_contexts = "'" . implode("','", $exclude_contexts) . "'";
            }

            $where .= " AND $tasks_table.context NOT IN($exclude_contexts)";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tasks_table.*, $task_status_table.key_name AS status_key_name, $task_status_table.title AS status_title,  $task_status_table.color AS status_color, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS assigned_to_user, $users_table.image as assigned_to_avatar, $users_table.user_type,
                    ...
                    $ticket_table.display_id AS ticket_display_id,
        ...
        LEFT JOIN $orders_table ON $tasks_table.order_id = $orders_table.id
        LEFT JOIN $estimates_table ON $tasks_table.estimate_id = $estimates_table.id
        LEFT JOIN $proposals_table ON $tasks_table.proposal_id = $proposals_table.id
