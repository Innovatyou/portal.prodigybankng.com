...
class Contracts_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $expiring_soon = $this->_get_clean_value($options, "expiring_soon");
        if ($expiring_soon) {
            $today    = get_today_date();
            $end_date = add_period_to_date($today, 1, "months");

            $where .= " AND $contracts_table.status = 'accepted' AND ($contracts_table.valid_until BETWEEN '$today' AND '$end_date')";
        }
