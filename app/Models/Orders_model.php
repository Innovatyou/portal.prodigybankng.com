...
class Orders_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $orders_table.*, $clients_table.currency, $clients_table.currency_symbol, $clients_table.company_name, $projects_table.title as project_title,
           (SELECT GROUP_CONCAT(',', $invoices_table.id, '--::--', $invoices_table.display_id) FROM $invoices_table WHERE $invoices_table.deleted=0 AND $invoices_table.order_id=$orders_table.id) AS invoices,
    ...
    function get_order_total_summary_from_verification_data($verification_data = array()) {
        $taxes_table = $this->db->prefixTable('taxes');
        $clients_table = $this->db->prefixTable('clients');
        ...
        $result = new \stdClass();
        $result->order_subtotal = $order_subtotal;
        ...
        // process taxes
        $tax_id = get_array_value($verification_data, "tax_id");
        if ($tax_id) {
            $tax_sql = "SELECT $taxes_table.* FROM $taxes_table WHERE $taxes_table.id=$tax_id";
            $tax = $this->db->query($tax_sql)->getRow();

            $result->tax_percentage = $tax->percentage;
            $result->tax_name = $tax->title;
            $result->tax = 0;

            if ($tax->percentage) {
                $result->tax = $order_subtotal_for_taxes * ($tax->percentage / 100);
            }

            $order_total += $result->tax;
        }


        $tax_id2 = get_array_value($verification_data, "tax_id2");
        if ($tax_id2) {

            $tax_sql2 = "SELECT $taxes_table.* FROM $taxes_table WHERE $taxes_table.id=$tax_id2";
            $tax2 = $this->db->query($tax_sql2)->getRow();

            $result->tax_percentage2 = $tax2->percentage;
            $result->tax_name2 = $tax2->title;
            $result->tax2 = 0;

            if ($tax2->percentage) {
                $result->tax2 = $order_subtotal_for_taxes * ($tax2->percentage / 100);
            }

            $order_total += $result->tax2;
        }

        $result->balance_due = number_format($order_total, 2, ".", "");
