...
class Tickets_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tickets_table.*, $ticket_types_table.title AS ticket_type, $clients_table.company_name, $project_table.title AS project_title, $task_table.title AS task_title,
              ...
              merged_ticket.display_id AS merged_with_ticket_display_id
        FROM $tickets_table
        ...
        LEFT JOIN $tickets_table AS merged_ticket ON merged_ticket.id = $tickets_table.merged_with_ticket_id
        $in_out_ticket_comments_count_join
        $join_custom_fieds    
    ...
    //get ticket last id
    function get_last_ticket_id() {
        $tickets_table = $this->db->prefixTable('tickets');

        $sql = "SELECT MAX($tickets_table.id) AS last_id FROM $tickets_table";
