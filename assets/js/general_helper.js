...
$(document).ready(function () {
    ...
    var addCommentLink = function (event) {
        ...
        if (clipboardData.indexOf('/#comment') > -1) {
            ...
            var newClipboardData = "#[" + taskId + "-" + commentId + "] (" + AppLanguage.comment + ") ";
    ...
    // Handle apply button
    $("body").on('click', '#ai-assistant-popover-custom-prompt-submit-btn', function () {
        var inputContent = $('#ai-assistant-custom-prompt').val().trim();
        showStaticPopover(window.lastStaticPopoverInitiator);
        ...
        runAIAssistant('custom_prompt', finalMessages, window.lastStaticPopoverInitiator);
    });

    // Handle quick ai assistant action
    $('body').on('click', '.quick-ai-assistant .quick-ai-assistant-action', function (e) {

        var $button = $(this);
        var action = $button.data('action');
        var $quickAiAssistant = $button.closest('.quick-ai-assistant');
        var buttonDom = getAiActionButtons(action);

        var popoverContent = "";
        if (action == "enter_custom_prompt") {
            popoverContent = `<div class="general-form form-group mb0">
                <textarea id="ai-assistant-custom-prompt" class="form-control" rows="5" cols="40" placeholder="${AppLanguage.enterPromptHere}"></textarea>
            </div>
            ${buttonDom}`;
        }

        showStaticPopover($quickAiAssistant, popoverContent);

        if (action == "improve" || action == "improve_selection" || action == "write_with_agent" || action == "enter_custom_prompt") {

            window.aiAssistantInputInstance = $quickAiAssistant.attr("data-quick-ai-assistant-target-input-field");

            if (action == "enter_custom_prompt") {
                setTimeout(function () {
                    $('#ai-assistant-custom-prompt').focus();
                }, 20);

                return; // don't call ajax for this action
            }
        }

        let finalMessages = [];

        if ($quickAiAssistant.attr('data-quick-ai-assistant-content-source')) {

            // for content type of source, there will be summarize, describe, key_points actions
            var text = $('body').find($quickAiAssistant.attr('data-quick-ai-assistant-content-source')).text().trim();
            // text = getPromptForAction(action, text);
            finalMessages.push({
                role: 'user',
                content: text
            });

        } else if ($quickAiAssistant.attr("data-quick-ai-assistant-list-source") && action == "write_with_agent") {

            var aiAssistantMaxMessagesCount = parseInt(AppHelper.aiAssistantMaxMessagesCount || "10");

            // for write_with_agent, there will be a list of messages
            let isDescending = $quickAiAssistant.attr("data-quick-ai-assistant-is-decending-list");
            var messagesAreaSelector = $quickAiAssistant.attr("data-quick-ai-assistant-list-source");

            if (messagesAreaSelector) {

                let $messagesArea = $(messagesAreaSelector);
                if ($messagesArea.length) {

                    let messages = $messagesArea.find(".quick-ai-assistant-message");
                    ...
                    if (aiAssistantMaxMessagesCount > 0 && messagesArray.length > aiAssistantMaxMessagesCount) {
                        messagesArray = messagesArray.slice(0, aiAssistantMaxMessagesCount);
                    }
                    ...
                    messagesArray.forEach(function (message) {
                        let $message = $(message);
                        finalMessages.push({
                            role: $message.attr("data-quick-ai-assistant-message-type"),
            ...
            // for improve, there will be an input field
            var text = $($quickAiAssistant.attr("data-quick-ai-assistant-target-input-field")).val().trim();
            //text = getPromptForAction(action, text);
            finalMessages.push({
                role: 'user',
                content: text
            ...
            finalMessages.push({
                role: 'user',
                content: window.aiAssistantSelection.text
            });
        }


        setTimeout(function () {
            runAIAssistant(action, finalMessages, $quickAiAssistant, $button.attr("data-agent-id"));
        }, 100);
    });

    $("body").on('click', '.ai-assistant-popover-close-btn', function () {
        $(".app-popover").remove();
    });
...
function attachDropzoneWithForm(dropzoneTarget, uploadUrl, validationUrl, options) {
    ...
    var postFilesDropzone = new Dropzone(dropzoneTarget, {
        ...
        dictDefaultMessage: AppLanguage.fileUploadInstruction,
        autoQueue: true,
        ...
        accept: function (file, done) {
            if (file.name.length > 200) {
                done(AppLanguage.fileNameTooLong);
            }

            if (file.size > AppHelper.uploadMaxFileSize) {
                done(AppLanguage.fileSizeTooLong);
                appAlert.error(AppLanguage.fileSizeTooLong);
            }
...
function showHideAIContextMenu(menuParentButtonId, action, value) {
    if (value) {
        $(menuParentButtonId).find("[data-action='" + action + "']").removeClass("hide");
    } else {
        $(menuParentButtonId).find("[data-action='" + action + "']").addClass("hide");
    }
}
...
var initSummernote = function ($instance) {
    ...
    options.lang = AppLanguage.localeLong;
    ...
    var settings = $.extend({}, {
        ...
        callbacks: {
            onInit: function () {
                $instance.on('summernote.mouseup', function (e, mouseEvent) {
                    if (editorData.aiContextMenuId) {
                        setTimeout(function () {
                            var range = $instance.summernote('createRange');
                            var selectedText = range.toString();

                            if (selectedText) {
                                window.aiAssistantSelection = {
                                    summernoteRange: range,
                                    text: selectedText
                                }
                            } else {
                            ...
                            showHideAIContextMenu(editorData.aiContextMenuId, "improve_selection", selectedText);
                        })
                    }
                });
            },
            ...
            onChange: delayAction(function () {
                ...
                if (editorData.aiContextMenuId) {
                    setTimeout(function () {
                        var value = getWYSIWYGEditorHTML(instance);
                        showHideAIContextMenu(editorData.aiContextMenuId, "improve", value);
                    })
                }

            }, 1000),
            onMouseUp: function (e) {
                console.log('Mouse up inside the editor:', e);
            },
...
function getAiActionButtons(action) {
    var submitBtnDom = ``,
        copyBtnDom = ``;

    if (action == "enter_custom_prompt") {
        submitBtnDom = `<div id='ai-assistant-popover-custom-prompt-submit-btn' class='btn btn-primary'><i data-feather='check' class='icon-16'></i></div>`;
    } else {
        copyBtnDom = `<div id='ai-assistant-popover-copy-btn' class='mr10 float-start clickable'><i data-feather='copy' class='icon-16'></i></div>`;
    }

    var insertBtnDom = ``;
    if (window.aiAssistantInputInstance && action !== "enter_custom_prompt" && action !== "improve_selection") {
        insertBtnDom = `<div id='ai-assistant-popover-insert-btn' class='mr10 float-start clickable'><i data-feather='check-circle' class='icon-16'></i></div>`;
    }

    var replaceBtnDom = ``;
    if (window.aiAssistantInputInstance && action == "improve_selection") {
        replaceBtnDom = `<div id='ai-assistant-popover-replace-btn' class='mr10 float-start clickable'><i data-feather='repeat' class='icon-16'></i></div>`;
    }

    return `<div class='custom-popover-button-area mt10 clearfix'>
        ${submitBtnDom}
        ${insertBtnDom}
        ${replaceBtnDom}
        ${copyBtnDom}
...
function runAIAssistant(action, messages, $popoverSelector, agentId) {
    // Set up the UI for streaming
    var hasReceivedFirstData = false;

    // Make streaming request using XMLHttpRequest
    const xhr = new XMLHttpRequest();
    xhr.open('POST', AppHelper.processQuickActionUrl, true);
    ...
    formData.append('agent_id', agentId ? parseInt(agentId) : 0);
    ...
    xhr.onprogress = function (e) {
        ...
        // If this is the first data we're receiving
        if (!hasReceivedFirstData && newData.trim()) {
            hasReceivedFirstData = true;


            // Create container for the response with a fixed height and scroll
            var content = '<div class="pt5 pb5"><div class="ai-response-container" style="white-space: pre-wrap; line-height: 1.5; min-height: 100px;"></div></div>';
            content = content + getAiActionButtons(action);
        ...
        for (const line of lines) {
            ...
            if (line.startsWith('response: ') || line.startsWith('error: ')) {
                ...
                try {
                    ...
                    var $responseContainer = $("#static-popup").find('.ai-response-container');

                    if (line.startsWith('response: ')) {

                        var data = JSON.parse(line.substring(10).trim());
                        if (data.content) {

                            $responseContainer.append(data.content);
                            $responseContainer[0].scrollTop = $responseContainer[0].scrollHeight;
                        }
                    } else {
        ...
        if ($responseContainer) {
            ...
            if ($responseContainer.height() >= 300 && $responseContainer.css('overflow-y') != 'scroll') {
                initScrollbar($responseContainer, {
                    setHeight: 300
                });
            }
        }
    ...
    xhr.onload = function () {
        var $responseContainer = $(".sp-body").find('.ai-response-container');
        if (xhr.status >= 200 && xhr.status < 300) {
            if (xhr.responseText.startsWith("error:")) {
                $responseContainer.append('<div class="error">Error: ' + (xhr.statusText || 'Can not connect the AI server.') + '</div>');
                $(".custom-popover-button-area").remove();
            }
            //Stream complete
        ...
        // Handle copy button
        $("body").on('click', '#ai-assistant-popover-copy-btn', function (e) {
            const el = document.createElement('textarea');
            el.value = $responseContainer.text();
            ...
            $btn.html('<i data-feather="check" class="icon-16"></i>');
            feather.replace();
        ...
        // Add insert functionality if there's an input field
        if (window.aiAssistantInputInstance) {

            $("#ai-assistant-popover-insert-btn").click(function () {

                if (AppHelper.settings.enableRichTextEditor === "1") {
                    insertHTMLintoWYSIWYGEditor(window.aiAssistantInputInstance, $responseContainer.text());
                } else {
                    //insert only text when rich editor isn't enabled
                    $(window.aiAssistantInputInstance).val($responseContainer.text());
                    $(window.aiAssistantInputInstance).trigger("input").focus();
                }

                $(".app-popover").remove();
            });

            $("#ai-assistant-popover-replace-btn").click(function () {
                const selection = window.aiAssistantSelection;
                if (!selection) {
                ...
                try {
                    if (selection.summernoteRange) {
                        selection.summernoteRange.pasteHTML(responseText);
                        ...
                        const newCursorPos = selection.start + responseText.length;
...
function showStaticPopover($initiator, content) {
    window.lastStaticPopoverInitiator = $initiator;
    $("body").staticPopup({
        width: 400,
        maxHeight: 400
    });
    ...
    $("body").staticPopup(
        "open",
        $initiator.offset().left,
        $initiator.offset().top,
        content
    );
}
