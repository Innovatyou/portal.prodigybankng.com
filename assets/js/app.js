...
$(document).ready(function () {
    ...
    moment.locale(AppLanguage.locale);
    ...
    (function ($) {
        $.fn.datepicker.dates['custom'] = {
            days: AppLanguage.days,
            daysShort: AppLanguage.daysShort,
            daysMin: AppLanguage.daysMin,
            months: AppLanguage.months,
            monthsShort: AppLanguage.monthsShort,
            today: AppLanguage.today
        };
    }(jQuery));
    ...
    $('body').on('click', '[data-act=ajax-modal]', function (e) {
        ...
        var ajaxModalId = $(this).data("ajax-modal-id");
        if (!ajaxModalId) {
            $(this).attr("data-ajax-modal-id", "ajax-modal-" + Math.random().toString(36).substring(2, 9));
            ajaxModalId = $(this).data("ajax-modal-id");
        }

        //check if we need to re-open the parent modal again.
        var reOpenParentModal = false,
            triggerSource = "";
        if ($(this).data("reopen-parent-modal") == "1") {
            reOpenParentModal = true;
            triggerSource = $("#ajaxModalContent").find(".modal-body").data("triggerd-from");
        }
        ...
        ajaxModalXhr = $.ajax({
            ...
            success: function (response) {
                ...
                $("#ajaxModalContent").find(".modal-body").attr("data-triggerd-from", ajaxModalId);

                if (reOpenParentModal) {
                    $("#ajaxModalContent").find(".modal-body").attr("data-trigger-on-modal-close", triggerSource);
                }
            ...
            error: function () {
                $("#ajaxModalContent").find('.modal-body').html("");
                appAlert.error(AppLanguage.somethingWentWrong, { container: '.modal-body', animate: false });
            }
        });
        return false;
    });
    ...
    $('#ajaxModal').on('hidden.bs.modal', function (e) {
        if (e.target && e.target.id === 'ajaxModal') {
            ...
            var triggerSource = $("#ajaxModalContent").find(".modal-body").attr("data-trigger-on-modal-close");
            $("#ajaxModalContent").html("");
            if (triggerSource) {
                setTimeout(() => {
                    $('[data-ajax-modal-id="' + triggerSource + '"]').trigger("click");
                }, 200);
        ...
        return this.each(function () {
            ...
            if (settings.list_data && settings.list_data[0]) {
                ...
                if (firstItem.dropdown_type == "MAX_DROPDOWN_ITEMS_REACHED" && firstItem.source_url) {
                    ...
                    select2Options.formatInputTooShort = function () { return AppLanguage.enterMinimum2characters }
...
class BuildFilters {
    ...
    prepareSelectionHandler() {
        ...
        dropdown += "<li role='presentation'><a href='#' class='dropdown-item select-all-btn'>" + AppLanguage.selectAll + "</a></li>";
        dropdown += "<li role='presentation'><a href='#' class='dropdown-item select-specific-btn'>" + AppLanguage.selectSpecific + "</a></li>";

        if (it.settings.selectionHandler.batchUpdateUrl) {
            dropdown += "<li role='presentation'><a class='dropdown-item batch-update-btn hide' data-act='ajax-modal' data-action-url='' " + dataPost + " data-title='" + AppLanguage.batchUpdate + "' type='button'>" + AppLanguage.batchUpdate + "</a></li>";
        }

        if (it.settings.selectionHandler.batchDownloadUrl) {
            dropdown += "<li role='presentation'><a class='dropdown-item download-selected-btn hide' data-action-url='' data-title='" + AppLanguage.downloadSelectedItems + "' type='button'>" + AppLanguage.downloadSelectedItems + "</a></li>";
        }

        if (it.settings.selectionHandler.batchDeleteUrl) {
            dropdown += "<li role='presentation'><a class='dropdown-item delete-selected-btn hide' data-action-url='' data-title='" + AppLanguage.deleteSelectedItems + "' type='button' data-action = 'delete-confirmation' data-reload-on-success = 'true'>" + AppLanguage.deleteSelectedItems + "</a></li>";
        }

        dropdown += "<li role='presentation'><a href='#' class='dropdown-item clear-selection-btn hide'>" + AppLanguage.clearSelection + "</a></li>";
    ...
    prepareSmartFilterDropdown() {
        if (this.settings.smartFilterIdentity) {
            ...
            var dropdown = "<div class='dropdown-menu w300'>"
                + '<div class="pb10 pl10">'
                + '<a class="inline-block btn btn-default manage-filters-button" data-act="ajax-modal" data-title="' + AppLanguage.manageFilters + '" ' + dataPostAttrs + '  type="button" data-action-url="' + actionUrl + '" ><i data-feather="tool" class="icon-16 mr5"></i>' + AppLanguage.manageFilters + ' </a>'
                + '<a class="inline-block btn btn-default clear-filter-button ml10 hide" href="#"><i data-feather="delete" class="icon-16 mr5"></i>' + AppLanguage.clear + '</a></div>'
                + '<input type="text" class="form-control search-filter" placeholder="' + AppLanguage.search + '">'
        ...
        if (this.settings.isMobile && !this.settings.smartFilterIdentity) {
            ...
            if (this.settings.dateRangeType || typeof this.settings.checkBoxes[0] !== 'undefined' || typeof this.settings.multiSelect[0] !== 'undefined' || typeof this.settings.radioButtons[0] !== 'undefined' || typeof this.settings.singleDatepicker[0] !== 'undefined' || typeof this.settings.rangeDatepicker[0] !== 'undefined' || typeof this.settings.filterDropdown[0] !== 'undefined') {
                ...
                var collapsePanelDom = "<div class='float-end filter-collapse-button'>\
                        <button title='" + AppLanguage.filters + "' class='dropdown-toggle btn btn-default mt0' data-bs-toggle='collapse' data-bs-target='#table-collapse-filter-" + this.randomId + "' aria-expanded='false'><i data-feather='sliders' class='icon-18'></i></button>\
                    </div>\
    ...
    prepareFilterFormShowButton() {
        if (this.settings.smartFilterIdentity) {
            ...
            var filterText = '<span class="add-filter-text ml5">' + AppLanguage.addNewFilter + '</span>';
            if (filters.length) {
                filterText = "";
            } else {
                this.$instanceWrapper.find(".smart-filter-dropdown-container").addClass("hide").closest(".filter-item-box").css("position", "absolute");
                ...
                $datePicker.datepicker({
                    ...
                    if (showClearButton) {
                        ...
                        if (!$(".datepicker-clear-selection").length) {
                            $(".datepicker").append("<div class='datepicker-clear-selection p5 clickable text-center'>" + AppLanguage.clear + "</div>");
    ...
    prepareSingleDatePicker() {
        ...
        if (typeof settings.singleDatepicker[0] !== 'undefined') {
            ...
            $.each(settings.singleDatepicker, function (index, datePicker) {
                ...
                datePicker.options.push({ value: "show-date-picker", text: AppLanguage.custom });
    ...
    prepareDynamicFilterDomAndEvents() {
        ...
        if (typeof settings.dynamicRanges !== 'undefined' && settings.dynamicRanges.length > 0) {
            ...
            buttonDom += '<div class="dropdown dynamic-range-filter">' +
                ...
                AppLanguage[settings.selectedDynamicRange] +
                '</button>' +
            ...
            $.each(settings.dynamicRanges, function (index, range) {
                ...
                buttonDom += '<li class="list-group-item clickable' + activeClass + '" data-range="' + range + '">' + AppLanguage[range] + '</li>';
            });
            ...
            var selectDynamicFilter = function (rangeName) {
                ...
                $dynamicDropDown.html(AppLanguage[rangeName]);
...
(function ($) {
    ...
    $.fn.appTable = function (options) {
        ...
        var defaults = {
            ...
            reloadHooks: [],
            customLanguage: {
                noRecordFoundText: AppLanguage.noRecordFound,
                searchPlaceholder: AppLanguage.search,
                printButtonText: AppLanguage.print,
                excelButtonText: AppLanguage.excel,
                printButtonToolTip: AppLanguage.printButtonTooltip,
                today: AppLanguage.today,
                yesterday: AppLanguage.yesterday,
                tomorrow: AppLanguage.tomorrow
            }
        };
        ...
        if (settings.summation && settings.summation.length && !settings.isMobile) {
            ...
            content += _prepareFooter(settings, 'current', AppLanguage.total);
            content += _prepareFooter(settings, 'all', AppLanguage.totalOfAllPages);
        ...
        var aLengthMenu = [[10, 25, 50, 100, -1], [10, 25, 50, 100, AppLanguage.all]];
        if (settings.serverSide) {
        ...
        var datatableOptions = {
            ...
            footerCallback: function (row, data, start, end, display) {
                ...
                if (settings.summation) {
                    ...
                    if (settings.isMobile || settings.mobileMirror) {
                        if (pageTotalContent) {
                            summationContent += "<div class='box'><div class='box-content strong'>" + AppLanguage.total + "</div></div>" + pageTotalContent;
                        }
                        if (allPageTotalContent) {
                            summationContent += "<div class='box'><div class='box-content strong'>" + AppLanguage.totalOfAllPages + "</div></div>" + allPageTotalContent;
                        }
        ...
        var undoHandler = function (eventData) {
            $('<a class="undo-delete" href="javascript:;"><strong>' + AppLanguage.undo + '</strong></a>').insertAfter($(eventData.alertSelector).find(".app-alert-message"));
        ...
        return (function () {
            ...
            function _template(message) {
                ...
                if ($('#ajaxModal').is(":visible")) {
                    zindex = parseInt($('#ajaxModal').css('z-index')) + 1;
                    zindex = "z-index: " + zindex;
                }


                return '<div id="' + this._settings.alertId + '" class="app-alert alert alert-' + className + ' alert-dismissible " role="alert" style="' + zindex + '">'
...
(function ($) {
    var spMethods = {
        init: function (options) {
            ...
            if (!$("#static-popup-overlay").length) {
                $("body").append(
                    '<div id="static-popup-overlay" class="sp-overlay">' +
                    '<div id="static-popup" class="sp-popup">' +
                    '<div class="sp-header">' +
                    'AI' +
                    '<button class="btn-close sp-close"></button>' +
                    '</div>' +
            ...
            $(document).off(".staticPopup");

            $(document).on("click.staticPopup", ".sp-close", function () {
                spMethods.close.call($(document));
            });
        ...
        setContent: function (content) {
            ...
            if (!content) {
                content = "<div class='loader-container inline-loader'></div>";
            }
            $(".sp-body").html(content);
            spMethods.reposition.call(this);
        ...
        open: function (x, y, content) {
            ...
            spMethods.setContent.call(this, content);
            $("#static-popup-overlay").show();
            spMethods.setPosition.call(this, x, y);
        },

        close: function () {
            $("#static-popup-overlay").hide();
        },
        ...
        reposition: function () {
            var data = $(this).data("staticPopup");

            if (!data) {
                return;
            }
            spMethods.setPosition.call(this, data.lastX, data.lastY);
        ...
        setPosition: function (x, y) {
            ...
            if (windowHeight - y < data.settings.minBottomSpace) {
                y = windowHeight - data.settings.minBottomSpace;
                if (y < data.settings.minTopGap) {
                    y = data.settings.minTopGap;
                }
            }
    ...
    $.fn.staticPopup = function (method) {
        if (spMethods[method]) {
            return spMethods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        }

        if (typeof method === "object" || !method) {
            return spMethods.init.apply(this, arguments);
        }
    };
